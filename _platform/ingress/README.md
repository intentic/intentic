...
Notice also that the edge forwards to **one** port, and the replay delivers to **one** service. It routes to a *sandbox*; which port inside that sandbox answers a given hostname is the container's own business, and its front ([_sandbox/front](../../_sandbox/front)) is what decides it.
...
- **The 502 says which kind of unreachable, and says it in a form the browser may read.** This process is the only party that can tell "your network is broken" from "that box is not dialled in" — it watched the request arrive and find nothing — and a cross-origin error is invisible to JavaScript unless the responder allows it, so the editor was left inferring from a bare `TypeError` and spinning forever. Every refusal here therefore carries `x-intentic-edge` (`no-tunnel`, `unknown-sandbox` when the platform has no such row, `dropped` when a held tunnel failed mid-flight) with the CORS headers that expose it, and a **preflight for a sandbox with no tunnel is answered 204** — refusing it would be accurate about the sandbox and would stop the browser ever sending the request that carries the verdict. `*` discloses nothing: the body is one constant sentence and no credential is ever accepted on it. Written in [edge-verdict.ts](../../_shared/sandbox-contract/src/protocol/edge-verdict.ts), read by the editor's connection machine.
...
- The wire format both halves agree on lives in [ingress-contract.ts](../../_shared/sandbox-contract/src/protocol/ingress-contract.ts); the HTTP/2-over-WebSocket data plane is [ingress-protocol.ts](../../_shared/sandbox-contract/src/protocol/ingress-protocol.ts), whose `openIngressSession` this package calls and whose `serveIngressSession` the daemon calls. Both halves run the same liveness rule, [tunnel-heartbeat.ts](../../_shared/sandbox-contract/src/protocol/tunnel-heartbeat.ts): this edge forgets a tunnel silent for 45 s, and the daemon redials one, since a path that dies without a FIN (a host that slept) tells neither end. A hosted machine's front door — the service the replay lands on — is declared beside the machine config in [sandbox-run/fly.ts](../../_shared/sandbox-run/src/fly.ts).
