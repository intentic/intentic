...
describe(`requesting a build: every refusal spends nothing`, () => {
    ...
    const outcome = async (prisma: PrismaClient, cfg: Config, over: Partial<Parameters<typeof requestHostedBuild>[3]> = {}) => {
        ...
        return {
            ...
            guards: (prisma.hostedMachine.updateMany as ReturnType<typeof jest.fn>).mock.calls.length,
            rows: (prisma.hostedBuild.create as ReturnType<typeof jest.fn>).mock.calls.length,
        };
    };
    ...
    it(`needs a machine we run`, async () => {
        expect(await outcome(fakePrisma({ hostedMachine: { findUnique: jest.fn().mockResolvedValue(null) } }), config())).toEqual(refused(`no-machine`));
    });
    ...
    it(`refuses a second build while one is in flight`, async () => {
        const prisma = fakePrisma({ hostedMachine: { findUnique: jest.fn().mockResolvedValue(machineRow({ buildingId: `b0` })) } });
        expect(await outcome(prisma, config())).toEqual(refused(`busy`));
    });

    it(`refuses past the owner's builds for the day`, async () => {
        const prisma = fakePrisma({ hostedBuild: { count: jest.fn().mockResolvedValueOnce(5).mockResolvedValue(0) } });
    ...
    it(`refuses when the platform is building as many as it may at once`, async () => {
        const prisma = fakePrisma({ hostedBuild: { count: jest.fn().mockResolvedValueOnce(0).mockResolvedValue(4) } });
        expect(await outcome(prisma, config())).toEqual(refused(`busy`));
    });
    ...
    it(`refuses when the platform's minutes for the day are spent, counting builds still running`, async () => {
        const prisma = fakePrisma({
            hostedBuild: {
                count: jest.fn().mockResolvedValueOnce(0).mockResolvedValue(3),
                aggregate: jest.fn().mockResolvedValue({ _sum: { minutes: 500 } }),
            },
        });
...
describe(`requesting a build: the start`, () => {
    ...
    it(`releases the guard and destroys the builder when the start fails after the machine was made`, async () => {
        ...
        const prisma = fakePrisma({ hostedBuild: { create: jest.fn().mockRejectedValue(new Error(`db down`)) } });
        await expect(requestHostedBuild(prisma, config(), logger, request())).rejects.toThrow(`db down`);
        const releases = (prisma.hostedMachine.updateMany as ReturnType<typeof jest.fn>).mock.calls.map(
...
describe(`the builder's report`, () => {
    const withBuild = (row: ReturnType<typeof buildRow>, over: Record<string, Record<string, ReturnType<typeof jest.fn>>> = {}) =>
        fakePrisma({ ...over, hostedBuild: { findUnique: jest.fn().mockResolvedValue(row), ...over[`hostedBuild`] } });
...
describe(`the reconcile`, () => {
    const building = (over: Record<string, unknown> = {}) =>
        fakePrisma({ hostedBuild: { findMany: jest.fn().mockResolvedValue([buildRow(over)]), findFirst: jest.fn().mockResolvedValue(null) } });
    ...
    it(`opens a guard whose build is no longer building`, async () => {
        stubFetch([]);
        const prisma = fakePrisma({
            hostedBuild: { findMany: jest.fn().mockResolvedValue([]), findFirst: jest.fn().mockResolvedValue(null) },
            hostedMachine: { findMany: jest.fn().mockResolvedValue([{ id: `h1`, buildingId: `b-old` }]) },
        });
        await reconcileHostedBuilds(prisma, config(), logger);
...
describe(`status and retention`, () => {
    it(`answers the latest build and what the machine runs`, async () => {
        const prisma = fakePrisma({
            hostedBuild: {
                findFirst: jest.fn().mockResolvedValue(buildRow({ state: `failed`, error: `the build exited 1`, log: `boom`, finishedAt: new Date(0) })),
            },
            hostedMachine: { findUnique: jest.fn().mockResolvedValue({ environmentHash: `old` }) },
        });
    ...
    it(`drops old rows but keeps the newest built one per machine`, async () => {
        const prisma = fakePrisma({
            hostedBuild: {
                findMany: jest.fn().mockResolvedValue([{ id: `keep-1` }, { id: `keep-2` }]),
