 ...
 * portal, one subscription read, and webhook signature verification. Hand-rolled over fetch rather than the
 * Stripe SDK, the stripe-api.ts precedent in the deploy engine: the platform's CLAUDE.md model is "as few
 * dependencies as the job allows", and the job here is four endpoints with stable shapes. Injectable fetch
 * for tests, like the trial pool's upstream. */
