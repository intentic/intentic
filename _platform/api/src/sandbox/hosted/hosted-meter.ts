...
const stopReason = async (prisma: PrismaClient, config: Config, ownerId: string, now: Date): Promise<string | undefined> => {
    const owner = await prisma.user.findUnique({ where: { id: ownerId }, select: { hostedSuspendedAt: true } });
    if (owner?.hostedSuspendedAt) {
        return `its owner's hosted lane is suspended`;
    }
    if (config.hosted.monthlyHours === 0) {
        return undefined;
    }
    ...
    return budget.metered && budget.usedMinutes >= budget.allowanceMinutes + config.hosted.overBudgetGraceMinutes
        ? `its owner's free hours are spent`
        : undefined;
};
...
export const stopOverBudgetHosted = async (
    ...
    for (const [ownerId, machines] of byOwner) {
        // oxlint-disable-next-line eslint/no-await-in-loop -- sequential sweep, gentle on the API
        const why = await stopReason(prisma, config, ownerId, now);
        if (why !== undefined) {
            // oxlint-disable-next-line eslint/no-await-in-loop -- sequential sweep, gentle on the API
            stopped += await stopOwnerMachines(config, logger, machines, why);
        }
    }
    return { stopped };
};
