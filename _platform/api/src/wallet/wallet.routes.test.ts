...
const fakePrisma = (seed?: { wallets?: StoredWallet[]; payments?: StoredPayment[] }) => {
    ...
    const walletDelegate = {
        findUnique: jest.fn(async ({ where }: { where: { userId_network: { userId: string; network: string } } }) => {
        ...
        create: jest.fn(async ({ data }: { data: Omit<StoredWallet, `id` | `perPaymentMaxUsd` | `dailyCapUsd`> }) => {
            const row = { id: `wallet-${(next += 1)}`, perPaymentMaxUsd: `1.00`, dailyCapUsd: `5.00`, ...data };
            wallets.push(row);
            return row;
        }),
        update: jest.fn(async ({ where, data }: { where: { id: string }; data: Partial<StoredWallet> }) => {
    ...
    const paymentDelegate = {
        findMany: jest.fn(async ({ where }: { where: { walletId: string; day: string } }) =>
            payments.filter((row) => row.walletId === where.walletId && row.day === where.day).map((row) => ({ amountUsd: row.amountUsd })),
        ),
        create: jest.fn(async ({ data }: { data: Omit<StoredPayment, `id`> }) => {
    ...
    const prisma = {
        ...
        $transaction: jest.fn(async (run: (tx: unknown) => Promise<unknown>) => run({ wallet: walletDelegate, walletPayment: paymentDelegate })),
    };
