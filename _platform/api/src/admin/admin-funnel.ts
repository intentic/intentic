...
// Every stage counts distinct accounts via Prisma relation filters, so each is a superset of the next and conversion is
// a subtraction. `setupEngaged` unions the lanes' different stamps; narrower would undercount a lane.
...
export const adminFunnel = async (prisma: PrismaClient, now: () => Date = () => new Date()): Promise<AdminFunnel> => {
    ...
    const [total, today, last7, last30, recentSignups, withSandbox, setupEngaged, connected, activeLast7] = await Promise.all([
        ...
        // 30 days of signups is a bounded read at this scale; bucketed in JS rather than teaching Prisma date_trunc.
    ...
    // Exactly SERIES_DAYS entries, zero-filled, oldest first, so the chart never has to guess at gaps.
...
// Sign-up to first announce, median hours, over accounts whose first activation landed in the last 30 days. Null when
// nobody activated in the window.
