...
- [src/components/](src/components) — `Card.vue`, `Page.vue`, `Code.vue`, `CodeField.vue`, `InfoHint.vue`, `AgentRunButton.vue`, and the markdown surface's own `Markdown.vue` / `MarkdownFigure.vue` (the figure switch) / `MermaidDiagram.vue` / `DagFigure.vue` (the lazily-imported graph branch).
