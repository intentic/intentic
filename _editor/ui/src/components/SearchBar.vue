...
// The clear button's room is reserved only when there is a button, so an unclearable field keeps its full width.
const inputClass = computed(() =>
    twMerge(
        ...
        clearable ? (variant === `panel` ? `pr-8` : `pr-7`) : ``,
    ),
);
...
<template>
    <div :class="rootClass">
        ...
        <button
            ...
            type="button"
            class="absolute top-1/2 flex -translate-y-1/2 items-center rounded text-2xs text-subtle transition-colors hover:text-content"
            :class="variant === `panel` ? `right-2` : `right-1.5`"
