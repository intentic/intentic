...
- **[src/composables/](src/composables)** — module-level singletons + vue-query wrappers:
  ...
  - `workspace/codeAnalysis.ts` + `codeAnalysisClient.ts` — one TextMate walk produces the comment-free view and import locations; a lazy dedicated worker runs it, and a small recent-result cache lets review warming, code-only stats, and an opened diff share that work.
