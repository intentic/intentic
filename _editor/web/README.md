...
The app runs in one of two **postures** ([src/environments/posture.ts](src/environments/posture.ts)). Everything
above describes the platform posture. The **local** posture is the same app embedded by a host application (an
editor extension, a CLI preview) over a daemon running its local profile on the same machine: no platform, no
sign-in, no daemon session — requests go plain, the one "sandbox" is seeded from the host-declared engine URL,
and a memory-history router serves just three areas (chat via a full-window dock slot, the agents board with its
drill-in, and the accounts surface — [src/local/](src/local)). The posture is declared by the host via
`window.env.local` and can never be entered from a hosted deployment.
