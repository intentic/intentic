...
- **[src/chat/](src/chat)** — the chat panel: `ChatPanel.vue` + its message view, tabs, and mode/provider menus. One instance with three homes — the docked column, the full-screen `/chat` area (`pages/ChatArea.vue`), its own pop-out window — and which form it wears is derived from where it is actually drawn (`composables/chat/chatSurface.ts`), never stored.
