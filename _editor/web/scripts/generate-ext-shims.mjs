...
// `__esModule` is the CJS-interop marker node's loader adds to a required module's namespace, not an export of the
// package; a runtime that does not add it would otherwise read this script's output as stale.
const isInterop = (name) => name === "__esModule";
...
export const extShims = async () =>
    Promise.all(
        targets.map(async (target) => {
            ...
            const names = (target.names ?? Object.keys(module))
                .filter((name) => name !== "default" && !isInterop(name) && isIdentifier(name))
                .toSorted();
            const lines = [
