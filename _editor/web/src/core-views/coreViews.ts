...
export const coreViews = (): readonly ViewRegistration[] => [
    {
        id: `infrastructure`,
        label: t(`views.coreViews.infrastructure`),
        surface: `rail`,
        ...
        detect: (repos) => {
            ...
            return intent === undefined ? [] : [{ key: intent.repo, title: t(`views.coreViews.infrastructure`), icon: `server`, repo: intent.repo }];
        },
        view: async () => (await import(`./infrastructure/InfrastructureView.vue`)).default,
    },
    {
        id: `live-status`,
        label: t(`views.coreViews.liveStatus`),
        surface: `rail`,
        ...
        detect: (repos) => {
            ...
            return target === undefined ? [] : [{ key: target.repo, title: t(`views.coreViews.liveStatus`), icon: `cloud`, repo: target.repo }];
        },
