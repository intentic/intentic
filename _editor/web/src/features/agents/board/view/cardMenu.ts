...
const filingRow = (agent: FleetAgent, here: boolean, act: MenuActions): MenuItem[] => {
    const filing = !here
        ...
        : agent.archivedAt !== undefined
          ? [{ label: t(`agents.agentsView.restore`), icon: `history`, command: () => act.restore([agent.id]) }]
          : canArchive(agent)
...
export const menuItemsFor = (agent: FleetAgent, facts: MenuFacts, act: MenuActions): MenuItem[] => {
    ...
    const groups: MenuItem[][] = [
        [
            ...
            ...(facts.peeked ? [{ label: t(`agents.agentsView.keepOpen`), icon: `pin`, command: () => act.keepAgent(agent) }] : []),
