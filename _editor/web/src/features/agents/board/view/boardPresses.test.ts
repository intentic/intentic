...
// Pins the presses that land on no agent card: a run's stop stays marked until the ledger says it stopped, filing a run
// away or back never throws at the row, a run's diagram is the workflows page, a refused wake re-reads the board, and a
// synthesis that cannot start says why on the notice strip.
...
describe(`a workflow run's row`, () => {
    ...
    it(`lets a stop the daemon refused go at once, rather than leave the row stuck`, async () => {
        const { workflows, presses } = pressesOf();
        workflows.stop.mutateAsync.mockImplementation(async () => {
            throw new Error(`it ended already`);
        });
        await presses.stopRun(run(`r1`, `running`));
        expect([...presses.stoppingRuns.value]).toEqual([]);
    });

    it(`files a run away and back without ever throwing at the row`, async () => {
        const { workflows, presses } = pressesOf();
        ...
        expect([workflows.archive.mutateAsync.mock.calls, workflows.unarchive.mutateAsync.mock.calls]).toEqual([[[`r1`]], [[`r2`]]]);
    });
