...
/* Whose conversation this is, in one token on the line it shares with the model and the branch: a colleague's given
   name, or for one nobody owns, the program that started it (a control token's label) or the conversation that spawned
   it (a link to the parent). The reader's own is never drawn — the caller decides that (sessionMark) — because a mark
   carried by nearly every card cannot tell two cards apart, and the row it used to take costs the lane a card. The full
   name is in the hover; taking it over or handing it on is in the card's menu. */
