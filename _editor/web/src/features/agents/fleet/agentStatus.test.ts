...
describe("attentionReason · every park names itself", () => {
     ...
     * the same way and nothing made them: the chip words put setup above a plain question and the verbs put it
     * below, under a comment on the verb list claiming the two agreed. A card carrying both — the registry
     * parks each card independently, so a question raised beside a setup ask is a real card — wore
     * "Setup needed" over a press reading "Answer": two instructions, one card, and the reader left to guess
