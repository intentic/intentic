// What a look may claim about a file it has only read the ends of. Pins the two claims that would be lies: a line
// count derived from a clipped window, and a fragment of a line drawn as though it were a line.
import { dropPartialFirst, dropPartialLast, type FileQuickLook, isAudioPath, isImagePath, quickLookLead, quickLookLines, quickLookOmitted } from "./fileQuickLook";

const look = (over: Partial<FileQuickLook> = {}): FileQuickLook => ({
    present: true,
    size: 12,
    head: `one\ntwo\nthree`,
    headBytes: 12,
...
it("counts lines only when the window reached the end of the file", () => {
    expect(quickLookLines(look())).toBe(3);
    // The same three lines, but from the first 12 bytes of a much longer file: three is now a fact about the window.
    expect(quickLookLines(look({ size: 900_000 }))).toBeUndefined();
});

it("has no line count for a file that is missing, binary or empty of text", () => {
    expect(quickLookLines(look({ present: false }))).toBeUndefined();
    expect(quickLookLines(look({ binary: true, head: `` }))).toBeUndefined();
    expect(quickLookLines(look({ head: ``, size: 0, headBytes: 0 }))).toBe(0);
    // A trailing newline ends the last line, it doesn't start another.
    expect(quickLookLines(look({ head: `one\ntwo\n` }))).toBe(2);
});
...
it("states the bytes neither window quotes", () => {
    expect(quickLookOmitted(look({ size: 100_000, headBytes: 8_192, tailBytes: 8_192 }))).toBe(83_616);
    // Head and tail together covering the file leave nothing to announce.
    expect(quickLookOmitted(look({ size: 12, headBytes: 12, tailBytes: 0 }))).toBe(0);
});
...
it("leads with the first lines that actually say something", () => {
    const lines = quickLookLead(look({ head: `\n   \n21:26 INFO starting  \n21:27 WARN retry\n21:28 ERROR gave up` }), 2);
