...
import { kindMeta, type QuickPick, type QuickPickSources, quickRows } from "./composerQuickPick";
...
const header = computed(() => {
    ...
    if (!settingsOffered.value) {
        return { label: t(`chat.chatMentionPopover.mentionFile`), icon: `paperclip` as const };
    }
    return props.filesOffered
        ? { label: t(`chat.chatMentionPopover.mentionFileChangeSetting`), icon: `paperclip` as const }
        : { label: t(`chat.chatMentionPopover.changeSetting`), icon: `sliders-h` as const };
});
...
<template>
    <ComposerPopover :icon="header.icon" :title="header.label" :busy="searching || pending">
        ...
        <div class="max-h-80 overflow-y-auto">
            ...
            >
                ...
                <template v-else-if="row.kind === 'drill'">
                    <Icon :name="kindMeta()[row.into].icon" class="shrink-0 text-2xs text-subtle" />
                ...
                <template v-else>
                    ...
                    <span class="w-14 shrink-0 text-2xs uppercase tracking-wide text-subtle">{{ kindMeta()[row.kind].badge }}</span>
        ...
        <p v-if="belowFloor" class="px-3 py-2 text-xs text-subtle">{{ t(`chat.chatMentionPopover.keepTypingToSearch`) }}</p>
        <p v-else-if="nothing" class="px-3 py-2 text-xs text-subtle">{{ t(`chat.chatMentionPopover.nothingMatches`, { query: token.query }) }}</p>
    </ComposerPopover>
</template>
