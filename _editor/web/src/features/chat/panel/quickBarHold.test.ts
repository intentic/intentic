...
describe(`the box`, () => {
    it(`is borrowed by a hover, kept by a press, and kept by a press after a hover`, () => {
        expect([walk(hover), walk(press), walk(hover, press), walk(press, hover)]).toEqual([
            { box: `borrowed`, transcript: `closed` },
            { box: `kept`, transcript: `closed` },
            { box: `kept`, transcript: `closed` },
            { box: `kept`, transcript: `closed` },
        ]);
    });
    ...
    it(`goes back with the pointer only when borrowed and empty`, () => {
        ...
        ]).toEqual([CLOSED, { box: `borrowed`, transcript: `closed` }, { box: `kept`, transcript: `closed` }, { box: `kept`, transcript: `closed` }]);
    });
    ...
    it(`folds on a press on the page unless it holds words, which it then holds only as a pointer would`, () => {
        ...
        ]).toEqual([CLOSED, { box: `borrowed`, transcript: `closed` }, CLOSED]);
    });
});

describe(`the transcript`, () => {
    it(`is borrowed by the eye's hover and goes with the pointer, before the box does`, () => {
        const lifted = walk(hover, { kind: `transcript`, keep: false });
        expect([lifted, stepQuickBar(lifted, { kind: `transcriptLeave` })]).toEqual([
            { box: `borrowed`, transcript: `borrowed` },
            { box: `borrowed`, transcript: `closed` },
        ]);
    });

    it(`is kept by the eye's press, with the box, and folded by the next`, () => {
        const kept = walk(hover, { kind: `transcript`, keep: true });
        expect([kept, stepQuickBar(kept, { kind: `transcriptLeave` }), stepQuickBar(kept, { kind: `transcript`, keep: true })]).toEqual([
            { box: `kept`, transcript: `kept` },
            { box: `kept`, transcript: `kept` },
            { box: `kept`, transcript: `closed` },
        ]);
    });
    ...
    it(`is kept with the box by a press on the content, so a selection survives the pointer leaving`, () => {
        expect(walk(hover, { kind: `transcript`, keep: false }, { kind: `press` }, { kind: `transcriptLeave` }, { kind: `leave`, words: false })).toEqual({
            box: `kept`,
            transcript: `kept`,
        });
    });
    ...
    it(`needs an open box to open over`, () => {
        expect(walk({ kind: `transcript`, keep: true })).toEqual(CLOSED);
    });
});

it(`undoes one thing per Escape, the transcript first, and folds everything on minimize`, () => {
    const open = walk(press, { kind: `transcript`, keep: true });
