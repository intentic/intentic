...
describe(`what cannot serve a turn, whatever its pools say`, () => {
    ...
    // A routed refusal without an account name drops the whole pool, and says who can give it back: an entitlement
    // refusal is somebody switching the seat off, whatever the pool's meters say.
    it(`takes a whole routed pool off the list when its refusal names no account`, () => {
        ...
        const capacity = chatCapacity([], NOW);
        expect(capacity.providers).toEqual([]);
        expect(capacity.out.map((entry) => entry.reason)).toEqual([`no seat`]);
        expect(capacity.blocked.map((entry) => [entry.fix, entry.count])).toEqual([[`admin`, 2]]);
    });
    ...
    it(`says what a provider is missing when nothing it holds can serve, and still dates a bench that will lift`, () => {
        ...
        expect(chatCapacity([], NOW).out[0]).toMatchObject({ reason: `needs connecting again`, reopensAt: undefined });
