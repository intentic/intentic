...
// turn's screenshot strip (ChatTurnShots), so both peek the same way.

// The fixed region the peek is drawn in (each edge's distance from the window's same edge), and where in it the
...
const GAP = 12; // px: between the picture and its peek.
const MAX_WIDTH = 1100; // px: cap so the peek stays a peek on very wide windows.
const CENTRE_SHARE = 0.85; // of the window, when no region beside or around the picture is worth using.
const MIN_SCALE = 1.5; // a region drawing the picture under this many times the thumb's size isn't a peek.
...
const drawnWidth = (box: PeekBox, viewport: { width: number; height: number }, aspect: number): number => {
...
// `aspect` is the picture's width over height (aspectOf); the peek is placed for it.
export const peekBox = (el: HTMLElement, aspect = WIDE): PeekBox => {
    ...
    const span = (centre: number): { left: number; right: number } => {
    ...
    // Left of or right of an edge box: the chat panel's first, so the peek covers workspace rather than the chat.
    const besides = (box: DOMRect): PeekBox[] => [
    ...
    // A cramped window: over its middle, the picture underneath included, since the peek never takes the pointer.
