 ...
 * checking out a branch does in an editor, and the chip in the tab row (WorkspaceScopeChip) keeps the answer
 * on screen, beside a tint on the workspace's own chrome that says "not the shared tree" without spending a
 * band on saying it.
 *
 ...
 * this second, and the two writers would lose each other's work with nothing to notice it. The reason sits on
 * the row the Edit button vacated (FileViewer's `scopedReadOnly`), where somebody finds out they wanted it,
 * rather than over every file whether or not anybody was going to type. Unsaved buffers in
