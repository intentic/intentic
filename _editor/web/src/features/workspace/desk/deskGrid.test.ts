...
import { isGridKey, moveInGrid } from "./deskGrid";
import { deskLayout, EMPTY_LAYOUT } from "./deskLayout";
import type { DeskGroup, DeskGroupKey } from "./deskOrder";
...
// Lays tiles out the way the desk does: each group's entries wrap at `columns`, groups stacked in order.
const laidOut = (columns: number, ...groupSizes: number[]) => {
    const keys: DeskGroupKey[] = [`folders`, `documents`, `pictures`, `media`];
    const groups: DeskGroup[] = groupSizes.map((size, at) => ({ key: keys[at]!, label: keys[at]!, entries: entriesOf(`${at}-`, size) }));
    return deskLayout(groups, true, { columns, tileHeight: 100, labelHeight: 30 });
};
...
describe(`arrow travel over the desk`, () => {
    ...
    it(`walks desk order sideways, across a group boundary too`, () => {
