...
import { byNaturalName, deskGroups, deskOrder, groupOf, labelsShown } from "./deskOrder";
...
const keys = (entries: readonly WorkspaceTreeEntry[]): string[] => deskGroups(entries).map((group) => group.key);
const names = (entries: readonly WorkspaceTreeEntry[]): string[] => deskOrder(deskGroups(entries)).map((entry) => entry.name);

describe(`which group a desk entry lands in`, () => {
...
describe(`when group labels are drawn`, () => {
    it(`shows none for a folder of one kind`, () => {
        expect(labelsShown(deskGroups([file(`a.ts`), file(`b.ts`)]))).toBe(false);
        expect(labelsShown(deskGroups([]))).toBe(false);
    });

    it(`shows them once there is a second kind to tell apart, folders included`, () => {
        expect(labelsShown(deskGroups([dir(`src`), file(`a.ts`)]))).toBe(true);
        expect(labelsShown(deskGroups([file(`a.ts`), file(`a.css`)]))).toBe(true);
    });
});
