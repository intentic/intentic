...
// Drawn by EditorPane in place of the empty state while the home preference is on. Reads the same tree the explorer
// draws, opens files into the same tabs, and does to its tiles what the tree does to its rows (useHomeActions). Owns
...
// its groups name. Typing on the home writes the same query, so the tree narrows with it.
const search = inject(HOME_SEARCH, undefined);
...
const go = (dir: string, toward: "forward" | "back"): void => {
    ...
    // The folder entered is the current entry: the tree marks and reveals it; nothing here is marked, since the home
    // is now inside it.
...
const up = (): void => {
    if (homeDir.value === workspaceDir.value) {
        return;
    }
    const from = homeDir.value;
...
const tileEl = (path: string): HTMLElement | undefined =>
    [...(scroller.value?.querySelectorAll<HTMLElement>(`[data-home-tile]`) ?? [])].find((el) => el.dataset[`homeTile`] === path);

// Lands on a tile by its place in home order: scrolls its band in, waits for it to mount, then takes focus there.
...
const onTile = (event: Event): boolean => event.target instanceof Element && event.target.closest(`[data-home-tile]`) !== null;
...
<template>
    ...
    <div
        ...
        @dragleave="onDragLeave($event, homeDir)"
        @drop="onDrop($event, homeDir)"
    >
        ...
        <div
            v-if="dropDir === homeDir || (dragging && over === homeDir)"
        ...
        >
            <span class="rounded-full border border-primary-500/60 bg-canvas/90 px-3 py-1 text-2xs font-medium text-primary-500 backdrop-blur">
                {{ dragging ? t(`workspace.homeView.moveInto`, { here }) : t(`workspace.homeView.dropToAddTo`, { here }) }}
            </span>
        </div>
