...
const file = (name: string, size?: number): WorkspaceTreeEntry => {
    const entry: WorkspaceTreeEntry = { name, path: `dir/${name}`, type: `file` };
...
describe(`what a hover can show`, () => {
    it(`lists a folder`, () => {
        expect(quickLookPlan(dir(`src`))).toEqual({ kind: `folder` });
    });

    it(`reads text with its grammar, or plain where none is known`, () => {
        expect(quickLookPlan(file(`index.ts`))).toEqual({ kind: `text`, lang: `typescript` });
        expect(quickLookPlan(file(`README.md`))).toEqual({ kind: `text`, lang: `markdown` });
        expect(quickLookPlan(file(`NOTES`))).toEqual({ kind: `text` });
    });

    it(`paints a picture the browser can draw`, () => {
        expect(quickLookPlan(file(`logo.png`))).toEqual({ kind: `picture` });
        expect(quickLookPlan(file(`icon.svg`))).toEqual({ kind: `picture` });
    });

    it(`plays a video`, () => {
        expect(quickLookPlan(file(`intro.mp4`))).toEqual({ kind: `video` });
        expect(quickLookPlan(file(`clip.webm`))).toEqual({ kind: `video` });
    });

    it(`draws a document as its own pages`, () => {
        expect(quickLookPlan(file(`brief.docx`, 40_000))).toEqual({ kind: `document` });
        expect(quickLookPlan(file(`notes.odt`, 40_000))).toEqual({ kind: `document` });
        expect(quickLookPlan(file(`memo.rtf`, 40_000))).toEqual({ kind: `document` });
    });

    it(`leaves a document too big to parse under the pointer`, () => {
        expect(quickLookPlan(file(`thesis.docx`, 40 * 1024 * 1024))).toEqual({ kind: `none` });
    });

    it(`shows nothing of bytes it cannot read`, () => {
        expect(quickLookPlan(file(`report.pdf`))).toEqual({ kind: `none` });
        expect(quickLookPlan(file(`site.zip`))).toEqual({ kind: `none` });
        expect(quickLookPlan(file(`raw.psd`))).toEqual({ kind: `none` });
        expect(quickLookPlan(file(`empty.png`, 0))).toEqual({ kind: `none` });
    });
});
...
describe(`the lines the card shows`, () => {
    ...
    it(`stops at the card's line budget`, () => {
        ...
        expect(quickLookLines(many, many.length, many.length).split(`\n`)).toHaveLength(QUICK_LOOK_LINES);
    });
});
