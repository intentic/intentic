import type { PublicList } from "@intentic/sandbox-contract";
...
// the one exception: a caller that can't rely on the push asks for a tick, only while waiting for a file to appear.
...
export function usePublicOutbox(live: MaybeRefOrGetter<boolean> = false) {
    ...
    const { query, error } = useSandboxQuery<PublicList>({
        queryKey,
        queryFn: () => sandboxRpc.public.list(),
