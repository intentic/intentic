...
import { hostEntry, osLabel } from "../../sandbox/devices/deviceFacts";
import { deviceState, deviceTone } from "../../sandbox/devices/deviceRows";
import { t } from "@intentic/ui/i18n";

// Machines this sandbox can see that no capability tile accounts for. A device arrives through either of two doors:
// a `host` capability, which is a tile, and a desktop-sync enrollment, which deliberately is not — a laptop syncing
...
// the vocabulary a tile states its connections in, so both screens are drawing the same answer.

/** One machine present through desktop sync alone, as the tile it would be connected on would state it. */
export interface DeviceConnection {
    /** Synthetic and prefixed: nothing keyed on capability ids may mistake this for an instance of one. */
    readonly id: string;
    /** The tile this machine belongs on, which is also where connecting it happens. */
...
// on the tile itself rather than on a settings form for something that does not exist.
export const isDeviceConnection = (id: string): boolean => id.startsWith(DEVICE_PREFIX);

// The machine back out of a row's id, for the navigation that carries its name to the tile's add form.
...
/**
 * Rows for machines reached by desktop sync that hold no host capability. A machine whose platform has no tile
 * (macOS today) is left out rather than pointed at a tile that does not exist; the Devices board remains the place
...
export const deviceConnections = (devices: readonly Device[], readAt: number): DeviceConnection[] =>
    devices.flatMap((device): DeviceConnection[] => {
        // A machine with a hostId is already a capability instance, and the tile lists it the ordinary way.
        if (device.hostId !== undefined || device.sync === undefined) {
            return [];
        }
        const entryId = hostEntry(device.platform);
