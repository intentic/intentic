...
test(`says a sandbox's share as one line, and only the parts somebody set`, () => {
    ...
    expect(resourcesSummary({ ...row, resources: share({ desired: { memoryGib: 20, cpus: null, privileged: false, gpu: false } }) })).toBe(
        `12 GiB · 4 CPUs · privileged · changes on restart`,
    );
});

// ic reports the shape a container runs with, whole, and the form reads it as it is: the owner's own ask, where an
// empty cap is the default even though docker enforces a derived number, and a switch the environment demands is not
// the owner's. An older agent reports no shape, and only then is docker's enforced share the reading.
test(`reads the shape ic reports as it is, and falls back to docker's share only without one`, () => {
    const shape = { memoryGib: null, cpus: 4, privileged: false, gpu: false };
    expect(runningShape(share({ shape }))).toEqual(shape);
    expect(runningShape(share())).toEqual(formFrom(share()));
});

test(`compares whole shapes field for field and says one in words`, () => {
    const shape = { memoryGib: 20, cpus: 1, privileged: false, gpu: true };
    expect(sameShape(shape, { ...shape })).toBe(true);
    expect(sameShape(shape, { ...shape, memoryGib: null })).toBe(false);
    expect(sameShape(shape, { ...shape, gpu: false })).toBe(false);
    expect(shapeSummary(shape)).toBe(`20 GiB memory · 1 CPU · not privileged · GPU`);
    expect(shapeSummary({ memoryGib: null, cpus: null, privileged: true, gpu: false })).toBe(`default memory · every CPU · privileged · no GPU`);
});
