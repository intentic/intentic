...
// test). `undefined` for a title the reading declines: the tile stays neutral rather than guessing.
...
const CATEGORIES: Readonly<Record<string, Omit<SessionCategory, "type">>> = {
    ...
    // ⇄ for reshaping what exists; not the cycle-arrows icon (that's the running-status arc, and a card can wear both).
