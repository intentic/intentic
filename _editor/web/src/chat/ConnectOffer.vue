...
import type { ConversationView } from "../composables/chat/useChat";
...
const props = defineProps<{ view: Pick<ConversationView, "provider" | "harness" | "selectProvider"> }>();

const provider = computed(() => props.view.provider.value);
const harness = computed(() => props.view.harness.value);
 ...
 * connected yet. The Sandbox ▸ Agent tab is where the handshake lives; the picked provider rides along as
 * `?connect=<provider>` so that tab opens on its card and flashes it into view. */
const connect = (target: AgentProvider) => {
    props.view.selectProvider(target);
...
const pick = (target: AgentProvider, ready: boolean) => (ready ? props.view.selectProvider(target) : connect(target));
</script>
...
<template>
    <div class="flex flex-col items-center gap-3 text-center">
        <!-- THE HEADLINE: the one way in that costs nothing. Absent only when there is no free channel left to
             ...
             place: it marks the ACTION as the free way in, which is the single thing this card is arguing. -->
        <template v-if="free">
