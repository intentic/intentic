...
/* TAKING THIS SANDBOX ELSEWHERE: the whole outbound half, on one card, because it is one job.
 *
 * It used to be two. "Move this sandbox" held the bundle and "Sandbox definition" held the `sandbox.toml`,
 ...
 * embeds one, see bundle.ts). So they are two buttons in one row now, and the row's own caption is the
 * choice between them.
 *
 ...
 *
 * TAKING ONE IN IS NOT HERE. That is ArrivalCard, and it is one card rather than three because a definition,
 * a bundle and a foreign assistant's home directory now arrive through one pipeline. */
 ...
 * definition carries this sandbox's own content at all, so it is the first thing the card has to say, not
        ...
        <div class="flex flex-col gap-4 p-5">
            <template v-if="canOperate">
                <!-- THE WORKSPACE REPO, as the one record it is: which repository, on which branch, and the two
                     things anyone does with a repository once it exists. Held back until the read lands,
                     because the two states are not equally cheap to be wrong about: "Not published" carries a
                     button that creates a repository, and drawing it for the beat before the daemon answers
                     offers that to owners who published months ago. -->
                <RowGroup v-if="workspace !== undefined" flat label="Workspace">
                    <Row v-if="published !== undefined" :icon="published.icon">
                        ...
                        <template #description>
                            Every definition carries it as <span class="font-mono">[workspace]</span>: notes, skills, personas, automations, designs
                            and drafts.
                        </template>
                    ...
                    <!-- Unpublished. What blocks the button IS the row's description, so the reader never
                         presses a greyed-out control to find out why it is grey. -->
                    <Row v-else icon="cloud-upload" title="Not published">
                        ...
                        <!-- WHAT STAYS BEHIND, at the moment it decides something. As standing prose it was four
                             ...
                             confirm step asks. `v-if` on the slot, not inside it: a slot that is passed is a
                             slot the row renders, margin and all. -->
                        <template v-if="confirmingPublish" #below>
                            <p class="text-2xs text-subtle">
                                Creates a private repository on {{ host }} and pushes <span class="font-mono">/work</span> to it. Nested repositories,
                                the reference shelf, credentials, browser sessions and <span class="font-mono">.env</span> files stay behind.
                            </p>
                        </template>
                    </Row>
                </RowGroup>

                <!-- THE TWO FIDELITIES, SIDE BY SIDE, which is the whole point of merging the cards: the
                     choice between a publishable document and a private archive is a choice, and a reader can
                     only make it when both are in front of them. The compare button rides along because it
                     reads the same document the first button writes. -->
                ...
                <!-- THE ONE DECISION THE EXPORT ASKS FOR, on the card's own surface. The lock is the state at a
                     ...
                     FULL-BLEED, because the whole row is the click target and the hover tint has to show that.
                     THE BLEED IS THE WRAPPER'S, NOT THE ROW'S: a <Row> carries `w-full`, and a negative side
                     margin on a box whose width is pinned to 100% SLIDES it instead of widening it. An
                     undecorated wrapper has no width of its own, so `-mx-5` widens it by the card's padding on
                     both sides and the row inside is 100% of THAT. -->
                <!-- `border-y` and not merely the bleed, which is the correction: on the card this row came
                     from it was the FIRST thing under the masthead, so the card's own top edge did the
                     separating. Here it follows a paragraph, and a full-bleed row with no rule reads as a
                     stray line rather than as the one control this half of the card offers. The band is what
                     the migration card's identical row always carried, for the same reason. -->
                <div class="-mx-5 border-y border-line">
                    <!-- `density="compact"` is what stops this reading as a SECOND card title: at the
                         comfortable tier a row's title is a hair off the masthead's own size. It is one option
                         on this card, and the compact tier is the app's word for that. -->
                    <Row
                        flush
                        as="label"
                        density="compact"
                        ...
                        title="Put secrets in the bundle"
                        class="px-5 py-2.5"
            ...
            <p v-else class="text-2xs text-subtle">Only the sandbox owner can export this sandbox.</p>
            ...
            <!-- THE ANSWER TO "where do I get it later": the exports that exist, whatever this tab has been
                 doing. BOTH ACTIONS ARE THE SAME AFFORDANCE, and that is the fix to a row that had two:
                 download was a bordered chip and delete a bare glyph, so the quieter of the two (throwing the
                 file away) was the one drawing a box. Neither needs a label because the row is one file and
                 these are the only two things anyone does to a file; the tooltip carries the word. -->
            ...
            <p v-if="canOperate && exports.length > 0" class="text-2xs text-subtle">
