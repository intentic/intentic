...
/* ADDING A COMPUTER TO THIS SANDBOX. Pick a folder, click Enable, paste the one-liner it reveals on the machine
 ...
 * on that machine, a warning when it went quiet, and a "Disable sync" that revoked EVERY paired computer at
 * once. All of it in the singular, under a list of the several computers that could disagree with it, because
 * the daemon published its enrollment list flattened into one holder and some names.
 *
 * Every one of those facts is per COMPUTER, so every one of them is now a row in the Computers list above, with
 * its switches beside it: pause under the folder, mirroring under the ports, unpair and revoke on the machine.
 * What is left here is the one thing that genuinely belongs to the sandbox rather than to any computer, because
 * it happens BEFORE there is a computer to put it on: minting the pairing.
 *
 ...
 * computer while their first holds sync; a member only ever sees the mirror flow, matching what the daemon would
 * grant them. */
...
/* WHETHER A COMPUTER ALREADY HOLDS FILE SYNC, read off the LIST rather than off a status call of this card's
...
const holder = computed<Computer | undefined>(() => computers.value.find((computer) => computer.sync?.mode === `sync`));
        ...
        <div class="flex flex-col gap-4 p-5">
            <template v-if="available">
                ...
                <p class="text-2xs text-subtle">
                    Pair another computer with this sandbox. Anything already paired is a row in
                    <b>Computers</b> above, with its folder, its ports and its switches.
                </p>
                <!-- Taking file sync over from the machine that holds it. Offered only when one does, and it
                     names it: this ends that computer's sync, which is the whole reason it is opt-in rather
                     than what Enable quietly does. -->
                <p v-if="takeover" class="text-2xs text-warning">
                    This takes over from {{ holder?.label ?? "the other computer" }}. Its file sync stops when you run the command below.
                </p>
                ...
                <template v-if="pairToken === undefined">
                    <div class="flex flex-wrap items-center gap-3">
                        <Button
                            :label="portsOnly ? 'Mirror ports to a computer' : takeover ? 'Take over on another computer' : 'Enable desktop sync'"
                            size="small"
                            :loading="minting"
                        ...
                        <button
                            ...
                            @click="startTakeover"
                        >
                            Sync from a different computer instead
                        </button>
                        <button
                ...
                <template v-else>
                    ...
                    <ul v-if="showFootprint" class="flex list-disc flex-col gap-1 rounded-lg bg-canvas py-2.5 pl-7 pr-3 text-2xs text-subtle">
                        ...
                        <li>
                            <span class="font-mono text-content">intentic-machine sync uninstall</span> removes all of it; that computer's
