...
/* BRINGING SOMETHING IN: one card, one file picker, one checklist, one report — for all four things that can
 * arrive in a sandbox.
 *
 ...
 * card asks it in the order it becomes cheap:
 *
 ...
 * the apply writes. Credentials are a second, separate consent, asked here — on the way IN — for every source,
 * including a bundle, whose export-time toggle used to ask a different person at a different moment. */
...
<template>
    <RowGroup label="Bring a sandbox in">
        <div class="flex flex-col gap-4 p-5">
            <template v-if="canOperate">
                <template v-if="plan === undefined">
                    <!-- THE OFFER THAT DELETES THE INSTRUCTIONS. A connected computer needs no archive, no
                         ...
                         than as a shortcut.
                         EVERY CONNECTED MACHINE GETS A ROW, not only the ones holding a setup: an owner whose
                         laptop is simply ASLEEP (the ordinary state of a laptop) would otherwise see a card
                         that had never heard of their computers and no reason to think reading one was
                         possible at all. A row saying "asleep, wake it and check again" is not a dead-end
                         offer; it is the difference between a feature that is missing and one that is
                         waiting. -->
                            ...
                            <template #title
                                ><span class="text-xs">{{
                                    host.found === undefined
                                        ? host.id
                                        : `Found a ${host.found === `hermes` ? `Hermes` : `OpenClaw`} setup on ${host.id}`
                                }}</span></template
                            >
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <Button :label="ready.length > 0 ? `Or choose a file` : `Choose a file`" size="small" :loading="planning" @click="chooseFile?.click()">
                        ...
                        <p class="text-2xs text-subtle">
                            A <span class="font-mono">sandbox.toml</span>, an environment bundle exported from another sandbox, or a packed Hermes or
                            OpenClaw folder. Nothing is written until you have read what it would land.
                        </p>
                    </div>

                    <!-- The last reader: neither a connected machine nor a file yet. One question with an
                         answer nobody has to look up, then exactly one command. -->
                    ...
                    <!-- One command, what it prints, where the file lands. Never two commands: the reader has
                         already told us which one is theirs. -->
                    <div v-else-if="guide" class="flex flex-col gap-3">
                        ...
                        <!-- The three cliffs, each answered where a reader hits it. Folded shut so the person
                             whose assistant runs right here never reads past the command. -->
                ...
                <!-- THE PLAN, and it is the same checklist whichever of the four this was. Nothing below this
                     writes until Apply. -->
                <template v-else>
                    ...
                    <!-- THE SECOND CONSENT, asked on the way IN for every source, and only when the artifact
                         actually holds values (`carriesSecrets`): a definition carries secret NAMES and never a
                         value, so asking would be a question about something that is not in the file.
                         Full-bleed on the wrapper, not the row — see MoveOutCard for why the margin cannot go
                         on a `w-full` row. -->
                    <div v-if="plan.carriesSecrets" class="-mx-5 border-y border-line">
                        <Row
                            flush
                            as="label"
                            density="compact"
            ...
            <p v-else class="text-2xs text-subtle">Only the sandbox owner can bring a sandbox in.</p>

            <!-- THE REPORT, written once. It was three near-identical blocks with three different headings for
                 the same list ("Finish the move", "Finish the arrival", "Finish the move" again). -->
            <template v-if="report">
                ...
                <!-- The failure group wears the tone its heading always did: <RowGroup>'s label is a slot
                     precisely so a group whose subject is a failure can say so without the component learning
                     about tones. -->
