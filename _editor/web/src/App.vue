<!-- Root shell: the router outlet, the signed-in session's runtime, and the app-global overlays — the single
     Upgrade dialog, the browser→sandbox Google sign-in gate, and the shell's model picker when something outside
     the chat asks for one. The persistent workspace chrome (rail + docked columns) lives in the `/` layout route
     (shell/WorkspaceShell.vue), so it survives navigation between the shell's child pages. All three overlays are
     ...
     is pre-warmed there (see Setup.vue) before the shell ever mounts, and the model picker is raised by whichever
