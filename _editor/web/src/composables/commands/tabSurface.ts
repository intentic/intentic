 ...
 * chord unambiguous — each surface registers its own command with `when: inTabSurface(<its surface>)`, and
...
export const inTabSurface =
    (surface: TabSurface): ((event: KeyboardEvent) => boolean) =>
    (event): boolean =>
        tabSurfaceOf(event) === surface;
