# intentic - connect THIS Windows device to your sandbox, so the agent can work on it: run PowerShell, read
# and write files inside the folders you allow, and see the screen. Runs as YOU (no admin): installs the one
# machine agent into %USERPROFILE%\.intentic\machine and registers a per-user logon entry so the device
# reconnects after a reboot. `intentic-machine device uninstall` removes it. Nothing is opened on your
...
if ([string]::IsNullOrEmpty($url) -or [string]::IsNullOrEmpty($pair)) {
    Write-Error 'SANDBOX_URL and PAIR_TOKEN are required (copy the command from the device''s card in your sandbox).'
    exit 1
}
