...
# per-user login entry so the computer reconnects after a reboot. `intentic-machine computer uninstall` removes
...
# to `intentic-machine computer setup`, which decides everything else — including moving an already-installed
...
#   curl -fsSL https://intentic.dev/computer | env SANDBOX_URL='https://sandbox-<id>.<zone>' PAIR_TOKEN='<token>' sh
#
# Required env:
...
exec $BIN computer setup --url "$URL" --pair "$PAIR"
