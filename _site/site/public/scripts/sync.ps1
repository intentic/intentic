...
# localhost (both powered by Mutagen). Runs as YOU (no admin): installs the one machine agent into
# %USERPROFILE%\.intentic\machine and registers per-user logon entries (the agent + the Mutagen daemon) so
# both resume after a reboot. `intentic-machine sync uninstall` removes everything.
#
# THIS IS A BOOTSTRAP SHIM: its whole job is to put an agent that understands `sync setup` on a machine
...
# WHICH HALF OF THE AGENT THIS SCRIPT CONNECTS, named once: `sync` here, `device` in device.ps1. The bootstrap
