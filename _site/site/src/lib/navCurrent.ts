...
/** A bar label is current for its whole prefix region, not just one page; a menu carries several prefixes since it can gather multiple sections. */
...
/** A menu row is current on its own page and every page its shelf covers (`covers` in `bookDestinations`); a row without a shelf falls back to just itself. */
