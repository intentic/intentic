...
- A field a newer daemon adds to a payload is optional in the contract, and the editor does not rebuild it from an older daemon's raw data. A view that needs it shows what the daemon did send, and a notice ([`SandboxOutdatedNotice.vue`](../../_editor/web/src/features/sandbox/overview/version/SandboxOutdatedNotice.vue)) says the sandbox is older, what the view lacks until it updates, and how to update it.
