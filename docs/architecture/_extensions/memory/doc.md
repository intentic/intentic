...
The notes an agent keeps for itself, where you can read and edit them.

```stats
{ "items": [
    {"label": "Lines", "value": "914"},
...
An agent that re-learns the same thing every session is expensive. Notes fix that — but unreviewable notes are a liability.

```dag
{ "title": "Its neighbours",
  "direction": "LR",
  "nodes": [
    {"id": "_extensions/memory", "label": "memory", "note": "this package", "accent": "3"},
  ...
  "edges": [
    {"from": "_extensions/memory", "to": "_libs/extension-api"},
    {"from": "_extensions/memory", "to": "_libs/extension-ui"},
    {"from": "_extensions/memory", "to": "_libs/sandbox-contract"},
    {"from": "_extensions/memory", "to": "_tools/tsconfig", "dashed": true},
    {"from": "_apps/web", "to": "_extensions/memory"}
  ] }
```
...
{ "title": "Size within Extensions (10 of 15)",
  "items": [
    ...
    {"label": "memory (this one)", "value": 914, "display": "914", "accent": "3"},
