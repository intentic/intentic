...
- A finding names what measured it by a free `source`, the repository's own word for it (a check's id, `lint`, a hook), with the command that shows it again and whether a later measurement can clear it (`recheckable`); the report's older `kind` enum still reads. Any repository's tooling can write the report.
- A push the repository's own pre-push hook refused reached no remote and left no report, so the daemon's push run ([`push-run.ts`](../../_sandbox/sandbox/src/git/ops/push-run.ts)) files it itself: a push marked `refused` whose one finding is the end of what the hook printed. The next push of the project, refused again or gone, settles it.
- Nothing is sent anywhere by itself. This is unlike a red land check: the push already went and the tree still works, so the owner chooses when to act. "Hand to an agent" (`POST /agents/push-fix`, [`push-fix.ts`](../../_sandbox/sandbox/src/agents/fix/push-fix.ts)) opens one isolated conversation with every open finding in the project, and the refused push's own card makes the same call: the daemon plans the attempt, and the editor only asks for it. The oldest push that still has an open finding sets its id, so pressing again continues the same attempt.
