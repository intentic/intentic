...
(`_shared/capability-catalog/src/index.ts`, the "Model endpoint" card): the owner installs Ollama or vLLM
...
| Big-weights precedent | `image-packs/whisper.Dockerfile` | Binary baked (small), weights downloaded into the workspace volume on first use (~1.6 GB and nobody complains) |
...
- **CPU build baked into the `standard` profile** (`image-packs/llamacpp.Dockerfile`, built like whisper's: same
...
- **Contract**: the `localmodel` kind, `LocalModelConfigSchema` (`model` HF path / `"custom"` + `url`,
  `gpu` on/off) and its union arm (`_shared/sandbox-contract/src/schemas/capabilities.ts`). Providers stay `endpoint/<id>`
...
- **Card + effects**: `_shared/capability-catalog`: curated select with RAM-honest labels, `when`-gated
