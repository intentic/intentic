...
> Requires **Node 24** and **pnpm 12** (`engines` names `24.21.0` and `12.4.1`). From this repo the CLI runs as `pnpm intentic deploy <command>` (the first call builds `dist`, then runs incrementally). The full authoring reference is [_tools/examples/deploy.config.ts](../../_tools/examples/deploy.config.ts).
