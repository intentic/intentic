...
| `_shared/sandbox-contract/src/schemas/providers/plan-limits.ts` | reply arm `payment_offer{approve}`, beside `capability_offer` and `credential_offer` |
...
| `_shared/capability-catalog` | `wallet` card, category `business`, guide covering funding and test mode |
