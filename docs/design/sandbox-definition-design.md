...
arrival.ts) with the schemas in `_shared/sandbox-contract/src/definition.ts` and `arrival.ts`, and the two
...
- **Derived, never stored** (the environment card's rule): there is no definition file the daemon keeps in
  ...
  A managed copy at the workspace root was tried and removed. A derivation is deterministic in what it
  DESCRIBES, not in when it changes: the document also carries host-instance facts (the trial endpoint's
  loopback URL, whichever image this sandbox booted from), so a file kept level with the sandbox rewrote
