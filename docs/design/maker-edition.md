# The maker edition: one reader, one product, across every handoff
...
how it is carried now, and what the site's `/maker` edition consists of.
...
sent to `/maker` ends up on those screens without ever being asked, whichever door they took.
...
- A **variant** is which design the *site* wears: the dark carved default, or `maker`, the light one. Decided in
...
- A **profile** is who is *arriving* at the app, which the app answers in more than paint: a look (scheme and
  skin), an audience, and a sandbox seed (`@intentic/constants` `profile.ts`). `maker` seeds `light`, `none`,
  `maker` in the browser, and `maker.sandbox.toml` in the sandbox (auto-accept, versioning).

They share their names (`default`, `maker`), and `variant.ts` is the one place that maps one to the other.
...
The third row is the one that was dropped. A reader on `/maker` who pressed *Download for Windows* never clicked a
...
the reader would have followed: `maker` is the maker profile, any other value is the site's way out (the default
...
Tagging the installer (a `-maker` in the filename read back by NSIS `$EXEFILE` or the AppImage's `$APPIMAGE`, a
...
`/maker/` was the light skin's front door: the same landing page, relit, `noindex`. For a while after that it was
...
**`/` is the developer's page and `/maker/` is intentic maker's** (`MakerLanding.astro`, content in
`site-content/src/landing.ts` `makerLanding`). Each page's words are its own by URL, from the first line to the
last FAQ question, and neither carries a hidden copy of the other. `/maker/` is indexed, in the sitemap,
canonical to itself, with its own title, description, Open Graph image, FAQPage and a `SoftwareApplication` of
its own (`buildMakerAppSchema`, `isRelatedTo` the developer product's). The two cross-link: one line under each
...
**What the cookie decides now is only the look of the shared pages.** A reader who came in through `/maker/`
reads docs, pricing and the feature pages in the light skin, with the nav's mark leading back to `/maker/` (a
runtime rewrite in `variant.ts`, since those pages are one cached document for both readers), and every link
into the app carrying `?profile=maker`. The words of the two product pages are never switched by it.

**The pictures** on `/maker/` are of the **maker recording**: a fourth demo mode (`_site/demo/src/mode.ts` `MAKER`,
`fixture/maker.ts`), the same app on a workspace of documents rather than code. One person, a small studio, a
...
fixture picks the maker's when the mode says so. The mode is read in the maker profile's look (light, unskinned, a
maker), written before first paint by the demo's own `index.html` and taken back on the way out. The maker page's
*Open the live workspace* opens it (`/demo/agents?mode=maker`).

`_tools/e2e/shots/capture.mts --maker` shoots it: `maker-*` files in the light set alone, drawn by `MakerShot.astro`
as single images, since the maker page is only ever light. The developer page keeps `ProductShot`'s dark/light
pairs.
...
- Only the landing pages are two products. The feature pages, docs, pricing and the download page are shared, in
  the maker skin for a maker reader, and their words are the developer's. The maker product's own sub-pages
  (`/maker/download/`, a maker features page) are the next step if the split proves out.
- The nav's menus are shared too: a maker reader sees Developers in the bar.
- The app's own words on a maker's screen are the maker design's business, not this document's, but two show on
  the maker recording's own shots: the *Ready to land* badge beside a maker's *Accept*, and *New agent* on the
