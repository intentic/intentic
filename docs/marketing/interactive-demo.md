...
| --- | --- | --- |
| Fleet board `/agents` | `GET /agents`, `/agents/search`, `/agents/{id}/rename`, `/archive`, `/unarchive`, `/{id}/seen`, `/{id}/react` | 9 agents, one in every lane state: `awaiting` (a question), `conflict`, two `running` (one delegating to subagents), two more that are steps of one workflow run, `ready`, `landed`, and an automation's overnight `idle`, how many of them a visitor meets is the mode's call (below). Two carry reactions from the presence roster's own pair, one of them Ada's own, so the strip shows a lit chip, a teammate's, and a press that answers for real |
...
| Model picker | `GET /claude/accounts`, `/grok/accounts`, `/translator/accounts`, `/providers`, `/providers/{provider}/models` | a connected Claude Max and a ChatGPT subscription in the translator, and a box that adds no provider of its own: without these the composer never leaves "Checking your AI accounts…" |
...
| Terminal | `GET /system/terminals` + the `/system/terminal` WebSocket | the featured turn's own tmux session, replaying its test run: xterm renders the ANSI for real |
