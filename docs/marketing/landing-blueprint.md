...
whole company of agents, a website Visitor chat. Fourteen sections, **16,254 px at 1440 wide, about
...
automations, Discord, Slack, IMAP, memory, pipelines and the Visitor chat webchat really are things you
...
and Visitor chat (`/features/connect/`, `/docs/visitor-chat/`). Adding any of those back as a full band is a
...
1. **Hero (`#hero`)**: the greeting eyebrow, the brand line kept verbatim ("More work. Less AI waste.
   ...
   **The bar is furniture inside the plate's frame, and it is the SAME bar on every page.** It is
   transparent everywhere, and what sits behind it is the plate: this page's hero paints its own, and
   every other page gets the same art at the same scale from `PageBackdrop` (veiled below the bar's
   height, so the carved band the bar lives in is untouched and the prose underneath is not fighting a
   devata). Measured, the bar on a docs page differs from the bar here by 0.27 of 255. A cut-down strip
   was tried first and thrown out: re-encoding changed the carving, and an absolutely-positioned overlay
   over the bar swallowed every click in the navigation.
...
- **Empower**: wire agents into your systems and onto your own website (was Capabilities, Visitor chat folded in).
...
- **Visitor chat folded into Empower, not deleted.** A website chat widget is one thing you empower an agent to
  do, not a product of its own: it is a block on `/features/connect/`, and `/docs/visitor-chat/` still owns the
  deep guide.
...
`hosted.ts`. The one paid thing is a hosted sandbox.
