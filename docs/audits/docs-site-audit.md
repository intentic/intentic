...
- **No troubleshooting anywhere.** "troubleshoot" and "not working" return nothing. The only failure-mode
  writing in the whole set is inside the Visitor chat page, where it is excellent ("This is the single commonest
  reason a freshly installed Visitor chat stays silent…") and is the model the rest of the docs should copy.
...
- **The Visitor chat page** is the best-structured page in the set: numbered steps, a check-that-it-worked step, the
...
| 9 | Add a troubleshooting page, modelled on the Visitor chat page's failure-mode writing | medium | Done. **Troubleshooting**, last on "Run a sandbox": symptom as the heading in the reader's own words, then cause, fix, and where the product already says so. Every entry is a failure this product's own docs or run contract describe. |
