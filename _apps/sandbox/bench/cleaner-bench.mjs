...
const FIXTURES = [
    {
        name: "find (path run)",
        command: "cd /work/intentic && find _apps/sandbox/src -name '*.ts'",
        exitCode: "0",
        raw: `${["agent", "agents", "logs", "sessions", "workspace", "acp", "usage"]
            .flatMap((dir) => Array.from({ length: 18 }, (_, i) => `_apps/sandbox/src/${dir}/${dir}-module-${i}.ts`))
            .join("\n")}\n`,
    },
    {
        name: "ls -la (long listing)",
        command: "cd /work/intentic/_apps/sandbox && ls -la src/agent",
        exitCode: "0",
        raw: `${[
            "total 628",
    ...
    },
    {
        ...
        command: "cd /work/intentic && grep -rn import _apps/sandbox/src",
        exitCode: "0",
        raw: `${Array.from(
            { length: 400 },
            (_, i) => `_apps/sandbox/src/mod-${i % 40}.ts:${i}:import { thing${i} } from "@intentic/sandbox-contract";`,
        ).join("\n")}\n`,
    },
    {
        ...
        raw: " M _apps/sandbox/bin/cleaners.mjs\n M _apps/sandbox/bin/agent-output-filter.mjs\n",
    },
    {
        name: "pnpm install",
    ...
    {
        name: "vitest (green)",
        command: "cd /work/intentic/_apps/sandbox && ./node_modules/.bin/vitest run",
        exitCode: "0",
