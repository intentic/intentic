...
// `_libs/sandbox-contract` is read, never assumed (`@intentic/lsp` lives in _apps/lsp, not _libs/lsp).
const workspacePackages = () => {
    const found = new Map();
    for (const group of ["_libs", "_apps"]) {
...
const bakedPackageNames = () => {
    const manifest = JSON.parse(readFileSync(join(REPO_ROOT, "_apps/sandbox/package.json"), "utf8"));
...
push(join(REPO_ROOT, "_apps/sandbox/dist"), `${SANDBOX_ROOT}/dist`);
