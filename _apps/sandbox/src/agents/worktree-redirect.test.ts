...
test("a main-root path becomes the same path inside the conversation's worktree", () => {
    expect(inWorktree("/work/intentic/_apps/web/src/main.ts", plan)).toBe("/history/worktrees/abc/intentic/_apps/web/src/main.ts");
...
test("the subtrees that mean the main checkout on both sides survive the redirect", () => {
    ...
    expect(inWorktree("/work/.intentic/claude/projects/x.jsonl", plan)).toBe("/work/.intentic/claude/projects/x.jsonl");
