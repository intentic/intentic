 ...
 * and none of it retries in a loop: the next tick past the TTL is the retry, which is a rate limit for free. */
...
// How much of a failing tool's output is kept as the reason. Enough to name the cause, short enough that a
// thousand-line stack trace cannot bloat the file every reader of this surface polls.
...
export const runProbe = async (spec: ProbeSpec, cwd: string, nowMs: number): Promise<ProbeResult> => {
    ...
    if (facts === undefined) {
        return finish({ state: "failed", reason: `could not read the tool's output: ${tail(stdout) || `it printed nothing`}` });
    }
