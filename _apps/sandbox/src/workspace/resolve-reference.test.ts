...
const WORKSPACE = [
    ...
    "_apps/web/src/pages/sandbox/SandboxSync.vue",
    "docs/pages/index.md",
    "src/foo.ts",
];
...
test("resolves the abbreviated path an agent writes once the area is established", async () => {
    expect(await resolve("pages/workspace/WorkspaceDesktop.vue")).toEqual({ path: "_apps/web/src/pages/workspace/WorkspaceDesktop.vue" });
});
