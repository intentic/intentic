...
const laptop: Capability = {
    id: "my-laptop",
    kind: "host",
    config: { platform: "windows", shell: "on", write: "off", screen: "on", control: "off" },
};
...
test("echoConfig renders the grant back and host holds no manifest secret", () => {
    ...
    expect(echoConfig(laptop, new Map())).toEqual({ platform: "windows", shell: "on", write: "off", screen: "on", control: "off" });
