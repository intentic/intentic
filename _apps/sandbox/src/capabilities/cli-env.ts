...
import { connectorEnv, connectorRegistry } from "./cli/connector-registry.js";
...
// connector's env template is expanded (connectorEnv) and every var suffixed with the instance id so two of the
// same provider coexist; the per-instance SKILL.md (written by cliHandler.apply) names the exact vars, so the
// agent never has to guess them. Provider data lives in an installed extension's connector (connectorRegistry).
export const cliEnvOf = async (host: ExtensionHost): Promise<Record<string, string>> => {
    const registry = await connectorRegistry(host);
    const env: Record<string, string> = {};
    for (const capability of await host.capabilities.list()) {
        if (capability.kind !== "cli") {
            continue;
        }
        const connector = registry.get(capability.config.provider);
        if (connector === undefined) {
            continue;
        }
        const suffix = envSuffix(capability.id);
        for (const [key, value] of Object.entries(connectorEnv(connector.spec, capability.config))) {
