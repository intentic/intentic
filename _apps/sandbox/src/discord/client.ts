...
// The sandbox's discord.js clients — one gateway connection per bot token, shared by the text listener and the
// voice session manager, each alive only while at least one consumer needs it. A module singleton map (like
// scheduler's inFlight): both consumers reach it directly. discord.js owns resume/backoff for a live session; a
// failed login clears that token's slot so the next reconcile tick retries.
...
// Live gateway state for the activity status route — a probe over the pool, no event plumbing.
