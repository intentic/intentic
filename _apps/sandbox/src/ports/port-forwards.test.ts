...
test("forwarding maps ports onto slots in order and is idempotent per port", async () => {
    ...
    expect(await forwards.forward(3000)).toBe("a");
    tick();
    expect(await forwards.forward(4000)).toBe("b");
    tick();
    expect(await forwards.forward(3000)).toBe("a");
    expect(forwards.slotOf(4000)).toBe("b");
    expect(forwards.targetOf("a")).toEqual({ port: 3000, scheme: "http" });
});
...
test("the probe's scheme lands on the slot — an https dev server is dialed as https", async () => {
    ...
    const slot = await forwards.forward(47145);
    expect(forwards.targetOf(slot)).toEqual({ port: 47145, scheme: "https" });
});
...
test("a full table evicts the least-recently-used slot — and proxy traffic counts as use", async () => {
    ...
    for (const [index] of PORT_SLOTS.entries()) {
        await forwards.forward(3000 + index);
        tick();
    }
    // Touch slot "a" (port 3000) through the proxy resolver; "b" becomes the LRU.
    forwards.targetOf("a");
    tick();
    expect(await forwards.forward(9999)).toBe("b");
