...
const routeFor = (
    ...
    const services = {
        ...
        hostHub: {
            call: overrides.call ?? (async () => ({ jsonrpc: "2.0", id: 1, result: { ok: true } })),
...
test("a request is forwarded verbatim and its answer returned unchanged", async () => {
    const call = vi.fn(async () => ({ jsonrpc: "2.0", id: 9, result: { tools: [{ name: "run_command" }] } }));
    const response = await post(routeFor({ call }), { jsonrpc: "2.0", id: 9, method: "tools/list" });
    expect(call).toHaveBeenCalledWith("laptop", { jsonrpc: "2.0", id: 9, method: "tools/list" });
...
test("a notification is delivered and answered 202 with no body", async () => {
    ...
    const response = await post(routeFor({ notify }), { jsonrpc: "2.0", method: "notifications/initialized" });
...
test("an asleep machine still completes the handshake, so it stays in the turn", async () => {
    const call = vi.fn();
    const response = await post(routeFor({ online: false, call }), { jsonrpc: "2.0", id: 1, method: "initialize" });
...
test("a live tools/list is remembered, which is what makes the offline answer possible", async () => {
    const app = routeFor({ call: async () => ({ jsonrpc: "2.0", id: 3, result: { tools: [{ name: "screenshot" }] } }) });
...
test("a tool call on an asleep machine is not answered locally", async () => {
    ...
    await post(routeFor({ online: false, call }), { jsonrpc: "2.0", id: 5, method: "tools/call", params: { name: "describe" } });
