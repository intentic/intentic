...
test("pushScopes reaches a connected machine and reports when there is nobody to reach", async () => {
    ...
    const machine = fakeMachine();
    hub.attach("laptop", machine.connection);
    expect(await hub.pushScopes("laptop", { shell: "on", write: "off", screen: "on" })).toBe(true);
    expect(machine.client.setScopes).toHaveBeenCalledWith({ shell: "on", write: "off", screen: "on" });
    expect(await hub.pushScopes("desktop", { shell: "on", write: "off", screen: "on" })).toBe(false);
});
