 ...
 * link does it, and _apps/host/src/link.test.ts proves it over a real handler.
 *
...
test("pushScopes reaches a connected machine and reports when there is nobody to reach", async () => {
    ...
    const machine = fakeMachine();
    hub.attach("laptop", machine.connection);
    expect(await hub.pushScopes("laptop", { shell: "on", write: "off", screen: "on", control: "off" })).toBe(true);
    expect(machine.client.setScopes).toHaveBeenCalledWith({ shell: "on", write: "off", screen: "on", control: "off" });
    expect(await hub.pushScopes("desktop", { shell: "on", write: "off", screen: "on", control: "off" })).toBe(false);
});
