...
/* The terminals WORK runs in — the agent's shells (`agent-<sdk session>`) and the daemon's job sessions
...
export function useWorkTerminals(): { rows: ComputedRef<WorkTerminalRow[]>; showWorkTerminals: Ref<boolean> } {
    ...
    const rows = computed<WorkTerminalRow[]>(() =>
        sessions.value
            .filter((session): session is TerminalSession & { kind: "agent" | "job" } => session.kind === `agent` || session.kind === `job`)
