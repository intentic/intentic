...
//
// The rule that keeps the strip clean: the terminals WORK runs in (an agent's Bash shell, a daemon job) are
...
//
// xterm is mocked wholesale: every case here is about which names reach `order`/`groups` and which one is
...
vi.mock("./terminalSession", () => ({
    createTerminalSession: (name: string) => ({ name }),
