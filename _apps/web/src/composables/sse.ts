// Low-level SSE framing shared by the daemon stream consumers — the intentic ndjson reads (intenticStream.ts)
// and the chat agent stream (chat/conversation.ts). The daemon emits SSE frames separated by a blank line, each
// carrying one `data: <JSON>` line. Kept protocol-only (no domain shapes) so both surfaces share one framer.
