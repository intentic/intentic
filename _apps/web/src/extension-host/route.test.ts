...
describe(`flattenQuery`, () => {
    it(`reads scalars straight through`, () => {
        expect(flattenQuery({ doc: `_libs/graph`, repo: `intentic` })).toEqual({ doc: `_libs/graph`, repo: `intentic` });
    });
    ...
    it(`takes a repeated key's FIRST value, since a view's state is singular`, () => {
        ...
        // later append added.
        expect(flattenQuery({ doc: [`_libs/graph`, `_apps/web`] })).toEqual({ doc: `_libs/graph` });
    });
...
describe(`mergeQuery`, () => {
    ...
    it(`adds a key that was not there`, () => {
        expect(mergeQuery({}, { doc: `_libs/graph` })).toEqual({ doc: `_libs/graph` });
    });
