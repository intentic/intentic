...
describe(`hotspotAsk`, () => {
    ...
    it(`quotes the row's own figures, and the window they were counted over`, () => {
        const ask = hotspotAsk(hotspot({ path: `_apps/web/src/App.vue`, commits: 12 }), context({ rank: 3, window: `30d` }));
        expect(ask.prompt).toContain(`Refactor _apps/web/src/App.vue.`);
        ...
        expect(ask.prompt).toContain(`\`iq hotspots --in _apps/web/src/App.vue\``);
    });
