...
/* One resource's reconcile verdict as a row: its id (mono) beside a status badge. The single row renderer
...
<template>
    <div class="flex items-center justify-between gap-2 rounded-md border border-line bg-canvas px-3 py-1.5">
        <span class="truncate font-mono text-2xs text-content">{{ id }}</span>
        ...
        <StatusBadge v-else :variant="statusVariant(action)" size="xs" class="shrink-0">
            {{ statusLabel(action, context) }}{{ reason ? " · " + reason : "" }}
        </StatusBadge>
    </div>
</template>
