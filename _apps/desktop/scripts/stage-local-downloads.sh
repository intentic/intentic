#!/usr/bin/env bash
# Build the desktop installers from this checkout and stage them into _apps/site/public/desktop/
...
STAGE="$ROOT/_apps/site/public/desktop"

# A configured updater pubkey makes `tauri build` demand the private key — local builds skip
# updater artifacts unless the signing env is present (same guard as release-build.sh).
CONFIG='{}'
