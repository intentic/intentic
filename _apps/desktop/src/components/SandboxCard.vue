...
const emit = defineEmits<{
    power: [slug: string, start: boolean];
    update: [slug: string];
    remove: [slug: string];
}>();
    ...
    <div class="flex flex-col gap-3 rounded-xl border border-line bg-canvas p-4">
        <div class="flex items-center gap-3">
            ...
            <!-- A sandbox reached over the user's own proxy has no sidecar at all, which is not the same fact
                 as a sidecar that is down — so the badge is absent rather than warning. -->
            <StatusBadge
        ...
        <div class="flex flex-wrap items-center gap-2">
            <Button
                size="small"
                :severity="sandbox.running ? `secondary` : undefined"
                :label="sandbox.running ? `Stop` : `Start`"
                :loading="busy === `power`"
                :disabled="busy !== null"
                @click="emit(`power`, sandbox.slug, !sandbox.running)"
            />
            <!-- Update is the button this whole app exists for after onboarding: the sandbox holds no host
                 Docker socket, so it cannot recreate its own container, and the browser can only ever hand
                 the user a command to paste on this machine. -->
            <Button
                size="small"
                severity="secondary"
                label="Update"
