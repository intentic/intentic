/* THE VAULT ENGINE AS A SECOND ENTRY POINT — everything in this directory that touches no filesystem.
 *
 * It exists for the callers that have to ANSWER for a vault without one behind them: the demo fixture, which
 ...
 * tree-shake, so anything exported there is shipped to every browser whether or not a vault is ever opened; the
 * engine belongs to the two halves that run it and to whoever stands in for them, not to the shell.
 *
 * read-vault.ts is absent for the opposite reason: it imports node:fs, and nothing here may. */
export { buildIndex, overviewOf, type BrokenLink, type VaultEdge, type VaultIndex, type VaultOverview } from "./index-vault.js";
export { parseNote, type NoteFile, type VaultNote, factsOf } from "./note.js";
