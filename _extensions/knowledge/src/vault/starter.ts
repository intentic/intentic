...
/* WHAT A BRAND-NEW VAULT GETS, and deliberately only one note.
 *
 * Seeding example people and example projects would fill somebody's knowledge base with facts about nobody, and
 * every one of them would have to be found and deleted before the vault said anything true. What a new vault
 ...
 * could carry it. Every word in it is editable — it is just a note, and the vault it describes is just a folder.
 *
 * The examples are FENCED, so the vault's own explanation of the link syntax does not fill the graph with links
...
the vault can no longer answer questions by relationship, which is most of what it is for.
...
A link to a note nobody has written yet is fine and deliberate: it is this vault's to-do list.
`;

// One note, at the conventional path. A list rather than a single value because what a starter vault should
