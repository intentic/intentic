...
import { buildIndex, type VaultIndex } from "./index-vault.js";
import type { NoteFile } from "./note.js";

/* THE VAULT ON DISK — the only file-touching module in this directory, so everything else stays pure and
 ...
 * one parser, one index, so the panel and the agent can never describe the same vault differently. */

// Where the notes live, relative to the workspace root, unless the owner points the setting elsewhere.
export const DEFAULT_VAULT = "knowledge";

// Directories a vault never keeps notes in — the editor's own state, a checkout, a dependency tree. Walked past
// rather than read: an Obsidian vault synced in here carries a .obsidian/ full of JSON that is not knowledge.
 ...
 * KB_VAULT (contributes.settings `env`), so the CLI needs no config file of its own; the backend reads the same
 * value out of the settings the daemon persists. Both fall back to the default folder, so a vault works before
 * anybody has opened the settings page. */
export const vaultRoot = (workspaceRoot: string, configured: string | undefined): string => {
    const folder = configured?.trim();
    if (folder === undefined || folder === "" || folder.split("/").includes("..") || folder.startsWith("/")) {
        return join(workspaceRoot, DEFAULT_VAULT);
    }
...
export const configuredVault = async (workspaceRoot: string): Promise<string | undefined> => {
    try {
        const raw = JSON.parse(await readFile(join(workspaceRoot, ".intentic/extension-settings.json"), "utf8")) as Record<
            string,
            Record<string, unknown> | undefined
        >;
        const value = raw["intentic.knowledge"]?.["vault"];
...
// The absolute path of one note, or undefined when the name escapes the vault or is not a markdown file. Notes
...
// Every markdown file in the vault, read. A vault that does not exist yet reads as empty rather than as an
// error: "no notes yet" is the first state of every vault, and the panel says so far better than a 500 does.
export const readVault = async (root: string): Promise<NoteFile[]> => {
        ...
        await Promise.all(
            entries.map(async (entry) => {
                ...
                try {
                    ...
                    // Raced a delete, or is not readable — the vault simply does not contain it this round.
                }
            }),
        );
    };
...
export const indexVault = async (root: string): Promise<VaultIndex> => buildIndex(await readVault(root));
