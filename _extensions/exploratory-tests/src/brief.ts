import { reportPath, resultPath, storyDir } from "./runs";
 ...
 * sandbox-wide setting (three bases, one owner-chosen), so "a slightly specialized exploratory-testing session"
 ...
 *
 * Four things it has to get right, each of which cost something real when it was missing:
 *
 ...
 *
 * 2. THE BROWSER MUST BE NAMED. Its ~20 tools are DEFERRED (isolatedBrowserSpec keeps them out of every turn's
 ...
 *
 * 3. SCREENSHOTS DO NOT GO WHERE THE MODEL ASKS. The daemon's PreToolUse hook rewrites every screenshot's
 ...
 *
 * 4. IT IS A TESTER, NOT A DEVELOPER. Left unsaid, a coding agent that finds a bug fixes it — which is the one
...
const HEADER = [
    `You are running an EXPLORATORY TEST of one user story against a running application.`,
    ``,
...
const method = (baseUrl: string): string =>
    [
        `## Method`,
        ``,
        `1. Read the story and write its acceptance criteria out as a checklist (ToolSearch \`select:TaskCreate,TaskUpdate,TaskList\`), one task per criterion. Keep it current — it is how the run is watched.`,
        ...
        `4. Then go off-script, because that is what makes this exploratory rather than a script: empty and invalid input, submitting twice, the back button, a reload mid-flow, a narrow viewport (\`browser_resize\`). Record what you tried even when nothing broke — "tried, held up" is a finding.`,
...
const output = (params: { readonly slug: string; readonly title: string; readonly dir: string; readonly report: string; readonly result: string }): string =>
    [
        ...
        JSON.stringify(
            {
                ...
                criteria: [{ text: "the criterion, quoted from the story", verdict: "pass | fail | untested", note: "why, in one line" }],
                steps: [{ n: 1, action: "what you did", expected: "what the story says", observed: "what happened", shot: "shots/01-....png" }],
                defects: [{ severity: "blocker | major | minor", summary: "one line", repro: "the shortest path to see it", shot: "shots/04-....png" }],
            },
            null,
            2,
        ),
...
export const briefFor = (input: BriefInput): string => {
    ...
    const sections = [
        ...
        [
            `## The story`,
            ``,
            `From \`${input.story.path}\` in the \`${input.repo}\` repository. Test what it says, not what the code does — ` +
        ...
        [`## The application under test`, ``, `Base URL: ${input.baseUrl}`, ``, `It is already running. Do not start, build, restart or reconfigure it.`].join(
            `\n`,
        ),
        tooling(`${dir}/shots`),
