...
const story: Story = { path: `app/docs/user-stories/auth/01-sign-in.md`, slug: `01-sign-in`, title: `Sign in`, group: `auth` };
...
describe(`briefFor`, () => {
    ...
    it(`states the screenshot redirect and the per-shot copy out of the shared output directory`, () => {
        ...
        expect(text).toContain(`/work/.intentic/exploratory/rabc/01-sign-in/shots`);
        expect(text).toMatch(/not in a batch at the end/);
    });

    it(`points both output files at this story's own run directory`, () => {
        const text = brief();
        expect(text).toContain(`/work/.intentic/exploratory/rabc/01-sign-in/report.md`);
        expect(text).toContain(`/work/.intentic/exploratory/rabc/01-sign-in/result.json`);
    });
