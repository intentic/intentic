 ...
 * composables reach the authenticated daemon transport, cache scoping and workspace facts through `host()` —
...
export const host = (): IntenticApi => {
    if (current === undefined) {
        throw new Error(`ext-exploratory-tests: host() called before activate()`);
    }
    return current;
};
