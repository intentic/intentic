...
const index = {
    repo: `intentic`,
    generatedAt: 1,
    entries: [entry(`_libs/graph`, 563), entry(`_libs/engine`, 1539), entry(`_apps/cli`, 6201)],
    edges: [
        { from: `_apps/cli`, to: `_libs/graph`, dev: false },
        { from: `_libs/engine`, to: `_libs/graph`, dev: false },
        { from: `_libs/graph`, to: `_tools/tsconfig`, dev: true },
    ],
    orphans: [],
    undocumented: [],
} as DocIndex;
...
const repoDoc = {
    repo: `intentic`,
    components: [{ id: `deploy`, name: `Deployment engine`, oneLiner: `x`, packages: [`_libs/graph`, `_libs/engine`, `_apps/cli`], accent: `5` }],
    glossary: [],
    reading: [],
    provenance,
} as RepoDoc;
...
describe(`packageFigures`, () => {
    it(`emits valid JSON in every fence`, () => {
        const markdown = packageFigures(`_libs/graph`, index, repoDoc);
    ...
    it(`draws dependents and dependencies in one direction, and marks a dev edge weaker`, () => {
        const dag = bodies(packageFigures(`_libs/graph`, index, repoDoc))[1] as {
        ...
        expect(dag.edges).toContainEqual({ from: `_apps/cli`, to: `_libs/graph` });
        expect(dag.edges).toContainEqual({ from: `_libs/graph`, to: `_tools/tsconfig`, dashed: true });
    });

    it(`gives every node its component's accent, so diagrams across the set agree`, () => {
        const dag = bodies(packageFigures(`_libs/graph`, index, repoDoc))[1] as { nodes: { id: string; accent: string }[] };
        expect(dag.nodes.find((node) => node.id === `_libs/graph`)?.accent).toBe(`5`);
    ...
    it(`omits the size comparison when there is nothing to compare against`, () => {
        ...
        const alone = { ...repoDoc, components: [{ id: `c`, name: `C`, oneLiner: `x`, packages: [`_libs/graph`], accent: `1` }] } as RepoDoc;
        expect(bodies(packageFigures(`_libs/graph`, index, alone))).toHaveLength(2);
    });
    ...
    it(`says nothing at all about a directory the index does not know`, () => {
        ...
        expect(packageFigures(`_libs/graph`, undefined, repoDoc)).toBe(``);
    });
});
