 ...
 * in a workspace with live agents in it is a real way to lose someone else's work.
 *
 ...
 * that says "staged". So `preflight()` reports every change outside the docs directory and the UI names the
 * number before the owner commits, rather than pretending to a precision the wire does not carry. A staged-paths
...
export function usePublish() {
    ...
    const preflight = async (repo: string): Promise<Preflight> => {
        ...
        try {
            ...
            foreign = status.files.map((line) => line.replace(/^\S+\s+/, ``)).filter((path) => !path.startsWith(`${DOCS_DIR}/`));
        } catch {
    ...
    const publish = async (repo: string, tails: readonly string[]): Promise<void> => {
        ...
        await api.sandbox.request(`/git/${encodeURIComponent(gitRepo(repo))}/commit`, {
            ...
            body: JSON.stringify({
                repo: gitRepo(repo),
                message: `docs: publish architecture documentation\n\n${paths.length} file${paths.length === 1 ? `` : `s`} under ${DOCS_DIR}/.`,
            }),
        });
