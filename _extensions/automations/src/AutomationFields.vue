...
<template>
    <div class="flex flex-col gap-3">
        ...
        <!-- THE TWO QUESTIONS AN AUTOMATION ANSWERS, SIDE BY SIDE: what fires it, and what it then does. They
             used to be one column forty fields tall, because the form lived in a 44rem modal and a column was
             the only thing that fit — the Doorbell branch alone is eight fields, and reaching the Prompt meant
             scrolling past every one of them without ever seeing the two halves together.
             Both callers now hand this component the whole page (72rem), so the split is the same in the create
        ...
        <div class="grid items-start gap-x-6 gap-y-3 md:grid-cols-2">
            ...
            <div class="flex min-w-0 flex-col gap-3">
                <!-- The one field nothing validates, and the one that has to agree with the trigger above it: a briefing
             about Discord messages on a CI trigger is a wake that reads a payload it was never told about. So the
             label says whose starting point is in the box while it is still a starting point, and a starter left
             over from another source is named with the swap beside it. -->
                <label class="ui-field">
                    ...
                    <!-- Six rows, not three. It is the longest thing on the form — a listener's starter is a
                         paragraph describing a payload — and it is the field the whole automation turns on, yet
                         it was the one showing three lines and a scrollbar. The column beside the trigger detail
                         is the shorter of the two on every branch, so the height is free. -->
                    <textarea
                        ref="promptInput"
                        v-model="form.prompt"
                        rows="6"
