...
<template>
    <Modal v-model:open="visible" size="md" :header="t(`frontDeskInstallDialog.install`)">
        <div class="flex flex-col gap-4">
            <!-- The deliverable, first and unmissable. Everything below it is about whether it worked. -->
            <div class="ui-field">
                <span class="ui-field-label">{{ t(`frontDeskInstallDialog.pasteIntoSiteBefore`) }} <span class="font-mono">&lt;/body&gt;</span></span>
                <div class="flex items-start gap-2 rounded-md border border-line bg-canvas px-3 py-2">
                    ...
                    <CopyButton :text="snippet" :aria-label="t(`frontDeskInstallDialog.copyEmbedSnippet`, { id: automation.id })" />
                </div>
            </div>

            <div class="ui-field">
                <span class="ui-field-label">{{ t(`frontDeskInstallDialog.allowedSites`) }}</span>
            ...
            <div class="ui-field">
                <span class="ui-field-label">{{ t(`frontDeskInstallDialog.loaded`) }}</span>
                ...
                <div v-else-if="installs.length === 0" class="flex items-start gap-2 rounded-md bg-overlay px-3 py-2.5 text-xs text-muted">
                    ...
                    <span v-if="isLoading">{{ t(`frontDeskInstallDialog.checking`) }}</span>
                    <span v-else>
                        {{ t(`frontDeskInstallDialog.noBrowserLoadedWidget`) }}
                    </span>
                </div>
                ...
                <template v-else>
                    ...
                    >
                        ...
                        <span class="min-w-0 flex-1">
                            ...
                            <span class="text-muted">{{ t(`frontDeskInstallDialog.askedTurnedAway`, { lastSeenAt: since(probe.lastSeenAt) }) }}</span>
                        </span>
                        <Button
                            size="small"
                            severity="secondary"
                            :label="t(`frontDeskInstallDialog.allow`)"
                            :loading="adding === probe.origin"
                            :aria-label="t(`frontDeskInstallDialog.allowToLoadChat`, { origin: probe.origin })"
