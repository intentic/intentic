<!-- The muted line under a draft's brand mark: which platform it goes to, where on that platform, and one
     ...
     A PLACE, NOT AN ADDRESS. A reply draft's target is the URL of the thread it attaches to, and rendered in
     full that is 90 characters of slug in link colour: the loudest, least readable thing on the row, and it
     answers "which thread" no better than its own subreddit does. destinationOf (postText.ts) reduces it to
     the place and the relationship: "reply in r/ClaudeAI", and the address survives as the link and the
     tooltip, so the piece worth recovering is one hover or one click away. Targets that were already readable
     (`r/webdev`, `#releases`, `@ada@hachyderm.io`) are passed through untouched.
