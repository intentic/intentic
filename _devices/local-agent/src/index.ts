/* The plumbing every intentic CLI that lives on a USER'S OWN DEVICE needs, and none of what any of them does.
 *
 * Two agents ship to a user's machine, `@intentic/machine` (the sandbox's agent works on this device, and
 * files and ports are mirrored between the two) and `@intentic/acp-bridge` (an editor talks to the sandbox),
 * and they have nothing in common except how they are INSTALLED and how they STAY alive: state under
