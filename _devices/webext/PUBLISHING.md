...
| Size ceilings, so an import habit can't quadruple what is uploaded | `scripts/size-budget.mjs` |
...
pnpm --filter @intentic/webext package     # → _devices/webext/dist.zip
```
...
| `CHROME_WEBSTORE_PUBLISHER_ID` | **variable** | `73c95ab7-68ae-4cb8-b272-1b772e351890` — Publisher → Settings, and the GUID in every dashboard URL |
| `CHROME_WEBSTORE_ITEM_ID` | **variable** | `ipdmljdopicihjdlgiiplaiolngbinma` — an extension id is public, and a secret would only make the logs unreadable |
...
- **A third screenshot, from a live session.** Two are committed: the popup
  (`assets/store/popup-1280x800.png`, regenerated by `pnpm --filter @intentic/webext preview`) and a page with
  the action banner up (`assets/store/banner-1280x800.png`). The banner shot draws the real `flashBanner` on a
  real page rather than capturing a paired agent mid-run; a genuine session would also show the popup's
  activity log filling up, which nothing in this repository can fake.
