 ...
 * linked sandbox (device/connection.ts) and the mirror watcher that holds the SSH transports, the port
 ...
 *   - The device half runs for as long as its links exist: a connection's `done` resolves only when it is
 ...
 * tick, while the device half's link list is fixed at startup — which is why every `setup` and `uninstall`
 ...
 * CLI builds, the number the Devices row prints) is the new file's. The skew is invisible without this, and it
...
/* The device half's background tick: keep each local sandbox's next update downloaded
 * (device/auto-prepare.ts), so the update card offers a half-minute restart instead of minutes. Gated on
