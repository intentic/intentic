...
// Everything the device half persists lives in the agent's shared home (~/.intentic/machine, ../config.ts):
...
/* ONE SANDBOX THIS DEVICE ANSWERS TO. There are as many of these as the owner has connected.
 *
...
export interface HostLink {
    ...
    // The capability id on the sandbox, this device's name, and the prefix of its tools over there.
    readonly id: string;
    readonly token: string;
    readonly scopes: HostScopes;
}

/* WHAT `intentic-machine device setup` WRITES AND EVERY OTHER COMMAND READS BACK — a LIST, and that is the whole of this
 * change.
 *
 * It used to be one link, flat, and `setup` wrote it wholesale. So connecting a device to a second sandbox
 * silently disconnected it from the first, and the flow that does that is not a command anybody types: it is
 * the LAST STEP OF ONBOARDING. Setting up a new sandbox on a device that already had one took the device
 ...
 *
 * A device is a device. Which sandboxes may drive it is a list, held here, and each entry carries its own
 * grant. */
...
// "this device is connected to nothing" and "there is no config file" stop being two cases at every call site.
 ...
 * device is re-enrolled. Keyed on the url, which is the one field of a link that is the sandbox's identity
...
export const rememberScopes = async (sandboxUrl: string, scopes: HostScopes): Promise<void> => {
    await updateHostConfig((config) => {
        ...
        // A push from a sandbox this device does not answer to is dropped rather than written: it can only
