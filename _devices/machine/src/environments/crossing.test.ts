...
test("hands a distro the script as one argument of sh, starting in the distro's home", () => {
    ...
    const wsl = crossInterpreter({ kind: "wsl", distro: "Arch" }, script, undefined, "windows", () => false);
    ...
    expect(crossInterpreter({ kind: "wsl", distro: undefined }, "ls", "/home/radarsu/proj", "windows", () => false).args).toEqual([
        "--cd",
        "/home/radarsu/proj",
        "--exec",
        "sh",
...
test("refuses to cross into WSL from anything but Windows", () => {
    expect(() => crossInterpreter({ kind: "wsl", distro: "Arch" }, "ls", undefined, "native", () => false)).toThrow(/Only a Windows PC/);
    expect(() => crossInterpreter({ kind: "wsl", distro: "Arch" }, "ls", undefined, "wsl", () => false)).toThrow(/Only a Windows PC/);
});
...
test("runs PowerShell through interop from a distro, and maps a drive path to its mount", () => {
    ...
    const exists = (path: string): boolean => seen.has(path);
    const home = crossInterpreter({ kind: "windows" }, "Get-Date", undefined, "wsl", exists);
    ...
    const there = crossInterpreter({ kind: "windows" }, "Get-Date", "C:\\Users\\radar\\proj", "wsl", exists);
    expect(there.args.at(-1)).toBe("Get-Date");
    expect(there.cwd).toBe("/mnt/c/Users/radar/proj");
    expect(crossInterpreter({ kind: "windows" }, "Get-Date", "/mnt/d/data", "wsl", exists).cwd).toBe("/mnt/d/data");
    expect(() => crossInterpreter({ kind: "windows" }, "Get-Date", "/home/radarsu", "wsl", exists)).toThrow(/drive path/);
});
...
test("falls back to Windows PowerShell 5.1, and says when no PowerShell is reachable", () => {
    ...
    expect(crossInterpreter({ kind: "windows" }, "Get-Date", undefined, "wsl", (path) => path === legacy).command).toBe(legacy);
    expect(() => crossInterpreter({ kind: "windows" }, "Get-Date", undefined, "wsl", () => false)).toThrow(/interop/);
});

test("refuses to cross to Windows from a device that is not a distro", () => {
    expect(() => crossInterpreter({ kind: "windows" }, "Get-Date", undefined, "native", () => true)).toThrow(/Only a WSL distro/);
});
