...
// Everything the sync half persists lives in the agent's shared home (~/.intentic/machine, ../config.ts): its
// pairing list in a file of its OWN beside the device half's (see the shared config for why the two never
// share one), the SSH keypair Mutagen authenticates with, and the known_hosts its ssh writes. The one credential
// besides the key is the enrollment-minted sync token (scoped daemon-side to reading the ports list). The 0600
// floor comes from @intentic/local-agent: this file used to write the token with the process umask, because it
// was copied from the host agent's before that floor existed and nothing afterwards compared the two.
 ...
 * and it matters MORE in the merged agent: the resident loop's pid also vouches for the device half, so a
 * stalled sync loop under a live pid is otherwise invisible from every direction at once.
 *
 ...
 * rather than as running (see report.ts). */
 ...
 * not have. mirrorOff is this device's owner saying "not on my localhost" (see setMirrorOff).
 ...
 * syncing the folder the user was working in, with both the printed status and the evicted sandbox's own
 ...
 *
 * Mirroring is the one half of this agent that changes the device it runs on: a sandbox's dev server takes
 ...
 * restart, and the watcher reads it every tick. The sandbox's Devices view can still ask for it (the daemon
 ...
 *
 * Scoped to one pairing, because a device mirroring three sandboxes has three answers to this question. File
