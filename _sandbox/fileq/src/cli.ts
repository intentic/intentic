...
// die as fileq's own one-line error on stdout, not as a raw node stack an agent's `2>/dev/null` swallows
...
// Errors go to STDOUT: the reader is an agent, and `fileq … 2>/dev/null` is a reflex that would turn a read
// failure into something indistinguishable from an empty document (iq's transcript-audited lesson). The exit
...
try {
    ...
    process.stdout.write(`fileq: cannot start, ${detail}\nfileq: this is a broken install, NOT an empty document. Reinstall fileq and report it.\n`);
    process.exit(2);
}
