...
# carries whatever machine builds it; exported as one file (build-front.sh, `--output type=local`). The context is the
# front's own directory, and build-front.sh passes `relay` (_shared/relay) beside it, laid out as in the repository.
