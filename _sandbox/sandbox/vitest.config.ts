 ...
 * The map comes from the contract's own `exports` (@intentic/testing/aliases), so the subpath entries —
 * `/chores`, `/tunnel-ids`, `/session-names` — resolve to source too, and go on resolving when a file moves
 * INSIDE the package. Aliasing the src DIRECTORY instead worked only while every subpath was a file at the
 * top of src/, and failed as a load error in every suite the day they were grouped. Same reasoning as
 * _extensions/workflows/vitest.config.ts.
 *
...
export default defineConfig({
    test: {
        projects: [
            ...
            // drive something real. src/fences/engine-fence.ts says what it costs without one.
             ...
             * private socket under the whole project (src/fences/tmux-fence.ts says what it costs without
