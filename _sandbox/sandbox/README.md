...
- [src/auth/join-links.ts](src/auth/join-links.ts) — owner-minted links that put an outsider on the members list without the platform: the link carries a role, the visitor's Google sign-in carries their name, and redeeming (`POST /join`, the one route exempt from the bearer middleware) can do nothing but add that verified email. Hashed at rest, revocable, bounded by expiry and by PEOPLE rather than openings; refused outright while no owner is bound, since ownership is trust-on-first-use.
