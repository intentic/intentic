...
- [src/keyed/](src/keyed): the providers connected by pasting an API key (Meta, Z.ai). Each publishes an Anthropic Messages endpoint of its own, so the Claude Code loop is pointed straight at it with the user's key — no translator hop, no adapter, no new runtime, which is the same road an `anthropic`-protocol endpoint capability already takes. One store, one catalog reader and one module FACTORY serve all of them, because what differs between two keyed providers is two URLs and a seed list, both of which are on the spec row: a third is a row in the contract and a seed in src/keyed/keyed-provider.ts.
