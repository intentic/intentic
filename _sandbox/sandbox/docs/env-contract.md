...
| `PNPM_HOME` | `/usr/local/share/pnpm` | where the baked native pnpm lives; its `bin/` is on PATH |
...
layers above, plus two names: `AUTOMATION_PAYLOAD` when the trigger arrived with one (for a schedule gated on
