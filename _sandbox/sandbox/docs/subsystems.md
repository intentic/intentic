...
- [src/peers](../src/peers), the substrate under everything of the user's that DIALS this sandbox and then serves
  ...
  - [src/hosts](../src/hosts): `host-peer.ts` is the door (the grant is the capability's config, pushed down on
    ...
    before it crosses, `host-command-guard.ts`), `host-seed.ts` the card the setup flow creates for the machine
...
- [src/hosts/host-command-guard.ts](../src/hosts/host-command-guard.ts): the same pipeline over a command headed for
