...
const card = (id: string, capabilities: readonly string[]): Identity => ({ id, capabilities: [...capabilities] });
...
test("upsert appends, then edits by id; list + get reflect it without duplicating", async () => {
    ...
    expect((await store.list()).map((identity) => identity.id)).toEqual(["work", "personal"]);
...
test("an invalid card is skipped and reported; the rest of the cast survives", async () => {
    ...
    expect((await store.list()).map((identity) => identity.id)).toEqual(["work"]);
    expect(skipped).toEqual(["broken"]);
});

// The card is the committed half of an identity and must stay free of anything that would make committing it a
