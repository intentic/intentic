...
test(
    "the settle still photographs the page where it is now, not the top of the document",
    async () => {
...
test(
    "a page where nothing is happening settles into silence",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
...
test(
    "a paused view stays silent even when a frame arrives after the stop",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
...
test(
    "a text frame lands in whatever field the page has focused",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
...
test(
    "editing chords land as editing commands in the page",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
...
test(
    "a focused drop-down can be read out and picked from",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
...
test(
    "the selection is read back out of a field, and out of an embedded frame",
    async () => {
        const browser = await launch();
        if (browser === undefined) {
            return; // no browser on this box
        ...
        try {
            ...
            const embedded = await context.newPage();
            await embedded.goto(
                `data:text/html,${encodeURIComponent(`<iframe srcdoc="<p id='inner'>signed in as someone@example.com</p>"></iframe>`)}`,
            );
            const inner = await embedded.waitForSelector("iframe").then((handle) => handle.contentFrame());
