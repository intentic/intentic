...
test("rejects saved paths outside generated_images and symbolic-link sources or destinations", async () => {
    ...
    await expect(
        persistCodexImageArtifact({ workspaceRoot, codexHome, image: { id: "outside", result: "", saved_path: outside } }),
    ).rejects.toThrow("outside CODEX_HOME/generated_images");

    const link = join(codexHome, "generated_images", "linked.png");
    await symlink(outside, link);
    await expect(
        persistCodexImageArtifact({ workspaceRoot, codexHome, image: { id: "linked", result: "", saved_path: link } }),
...
test("rejects malformed base64, non-PNG content, and artifacts over 32 MiB", async () => {
    const { workspaceRoot, codexHome } = await fixture();
    await expect(persistCodexImageArtifact({ workspaceRoot, codexHome, image: { id: "bad", result: "%%%" } })).rejects.toThrow(
        "not valid base64",
    );
    await expect(
    ...
    await expect(
        persistCodexImageArtifact({ workspaceRoot, codexHome, image: { id: "large", result: "", saved_path: oversized } }),
    ).rejects.toThrow("exceeds 32 MiB");
});
...
test("rejects an artifacts directory that resolves outside the workspace", async () => {
    ...
    await expect(
        persistCodexImageArtifact({ workspaceRoot, codexHome, image: { id: "escaped", result: PNG.toString("base64") } }),
    ).rejects.toThrow("output directory is outside the workspace");
});
