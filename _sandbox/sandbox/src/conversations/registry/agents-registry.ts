...
// provider, harness and account are the one routing rule's (agent/providers/accounts/routing.ts), which never carries an account
...
export interface AgentsRegistry {
    ...
    // Makes a member answerable for the conversation; who may is the route's decision (conversations/ownership.ts). Leaves
    ...
    // (conversations/archive.ts owns that order).
