...
const rows: readonly Row[] = [
    ...
    {
        name: "a payment card raised on a live turn parks it like any other card",
        from: running(),
        event: frame({
            kind: "payment_offer",
            requestId: "pay-1",
        ...
        to: running({ parked: [{ requestId: "pay-1", kind: "payment_offer" }] }, { lastAt: NOW }),
        effects: BROADCAST,
    },
    {
    ...
    {
        name: "arming starts a hold now",
        from: idle(),
        event: { kind: "keep-warm-armed", until: 60_000 },
        to: { ...idle(), keepWarm: { since: NOW, until: 60_000, refreshes: 0 } },
        effects: BROADCAST,
    },
    {
        ...
        event: { kind: "keep-warm-armed", until: 80_000 },
    ...
    {
        name: "an ending with no reason is a person's stop, and clears the hold for every card",
        from: { ...idle(), keepWarm: { since: 1_000, until: 90_000, refreshes: 2 } },
        event: { kind: "keep-warm-ended" },
        to: idle(),
        effects: BROADCAST,
    },
    {
        name: "a person's stop clears an ended hold too",
        from: { ...idle(), keepWarm: { since: 1_000, until: 90_000, refreshes: 2, ended: { at: 3_000, reason: "cold" } } },
        event: { kind: "keep-warm-ended" },
        to: idle(),
        effects: BROADCAST,
    ...
    {
        name: "a turn that left a cache it can keep says so to every card, once",
        from: idle(),
        event: { kind: "keepable-noted", keepable: { budget: 9 } },
        to: idle({ keepable: { budget: 9 } }),
        effects: BROADCAST,
    },
    {
        name: "the same keepable noted again changes nothing",
        from: idle({ keepable: { budget: 9 } }),
        event: { kind: "keepable-noted", keepable: { budget: 9 } },
        to: idle({ keepable: { budget: 9 } }),
        effects: [],
    },
