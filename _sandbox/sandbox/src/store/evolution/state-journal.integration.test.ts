...
const readable = { id: "1-1", engine: 3, digest: "aaaa", version: "1.400.0", state: "open", startedAt: 1, entries: [{ path: "/work/a.json", preImage: null }] };
...
test("an episode this build cannot read costs that episode alone: logged, kept as written, the rest read", async () => {
    ...
    await writeFile(journalPath(root), JSON.stringify({ episodes: [readable, newer], moved: { "/work/old.json": 5 } }));
    const { seen, logger } = warnings();

    const journal = await readJournal(root, logger);

    expect(journal.episodes.map(({ id }) => id)).toEqual(["1-1"]);
    expect(journal.moved).toEqual({ "/work/old.json": 5 });
    ...
    expect(JSON.parse(await readFile(journalPath(root), "utf8"))).toEqual({ episodes: [newer], moved: { "/work/old.json": 5 } });
});
