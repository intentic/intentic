...
import { z } from "zod";
import { type DocumentSpec, documentKey, registeredDocuments } from "../documents.js";
...
// frozen shape still converts to what today's schema accepts (store/conversion-types.ts), so a change that would strand
...
interface FrozenShape {
    ...
    // Where the shape was first seen: a release tag for a seeded one, the day it was frozen otherwise.
...
// Where the contract lock has lived: the package moved in the directory overhaul of 2026-09-06.
const LOCK_PATHS = ["_shared/sandbox-contract/contract.lock.json", "_sandbox/sandbox-contract/contract.lock.json"];
...
const seed = async (shapes: Shapes, located: readonly Located[]): Promise<void> => {
    ...
    for (const tag of tags) {
        const text = LOCK_PATHS.map((path) => git("show", `${tag}:${path}`)).find((found) => found !== undefined);
        if (text === undefined) {
            continue;
        }
...
const checksSource = (shapes: Shapes, located: readonly Located[]): string => {
    ...
    return [
        ...
        'import type { Accepted, Converted, Fits, ReusedKeys } from "../conversion-types.js";',
        ...imports,
        "",
        ...blocks,
...
for (const { spec } of located) {
    append(shapes, documentKey(spec), { type: shapeOf(spec.schema), since: today });
}
