...
describe("which tools mount", () => {
    ...
    it("the engine brings the spawn/wait pair, and unattended keeps it: a child deadlocks nothing", () => {
        const spawn = async (): Promise<{ ok: true; id: string }> => ({ ok: true, id: "sub-x" });
        expect(Object.keys(cursorCustomTools(request({ spawn }), push()))).toEqual(["ask", "spawn", "wait"]);
        expect(Object.keys(cursorCustomTools(request({ spawn, unattended: true }), push()))).toEqual(["spawn", "wait"]);
    });
});
...
describe("what the pair does", () => {
    it("spawn relays the spec and answers with the child's id and the wait to follow it with", async () => {
        const specs: unknown[] = [];
        const spawn: NonNullable<AgentRequest["spawn"]> = async (spec) => {
            specs.push(spec);
            return { ok: true, id: "sub-brave-otter-a1b2" };
        };
        const tools = cursorCustomTools(request({ spawn }), push());
    ...
    it("spawn without a prompt refuses without calling the engine", async () => {
        let called = 0;
        const spawn: NonNullable<AgentRequest["spawn"]> = async () => {
            called += 1;
    ...
    it("wait parks on the same roster the harness tool reads, and returns the settled child", async () => {
        ...
        const spawn: NonNullable<AgentRequest["spawn"]> = async () => ({ ok: true, id: "sub-child-1" });
        const tools = cursorCustomTools(request({ spawn }), push());
