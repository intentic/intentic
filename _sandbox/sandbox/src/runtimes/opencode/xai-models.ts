...
export const discoverXaiModels = async (accessToken: string, fetchImpl: typeof fetch = fetch): Promise<string[]> => {
    ...
    }).catch(() => undefined); // allow(silent-catch): an unreachable probe names no model, the same as a refusal naming none
    if (response === undefined || response.ok) {
        return [];
    }
    // allow(silent-catch): a rejection whose body is not JSON names no model
