...
jest.mock("@anthropic-ai/claude-agent-sdk", async () => ({ ...claudeAgentSdkOriginal, query }));
// Credential resolution is covered elsewhere; here it returns ok, so only the run is under test.
jest.mock("../../agent/providers/harness-credentials.js", async () => ({
    ...harnessCredentialsOriginal,
    resolveHarnessCredentials: async () => ({ ok: true, credentials: {} }),
}));

// Loaded after the mocks: jest.mock binds at this point, and a static import would have evaluated the graph first.
