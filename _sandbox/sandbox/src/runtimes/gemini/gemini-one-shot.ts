...
/* GEMINI'S ONE-SHOT (agent/adapter.ts oneShot): one prompt in, one string out, on Gemini's own runtime, the
 * other road to the model, and it exists for one reason: THE CLAUDE CODE HARNESS CANNOT REACH GOOGLE.
 *
 ...
 *
 * The settings mirror the Claude Code helper's (claude/claude-one-shot.ts), for the same reasons stated there, no tools, no session left behind, its
 ...
 * argue with a caller asking for one line, the same reason the Claude Code helper wants an empty system prompt. Unlike
...
export const geminiOneShot = async (services: Pick<Services, "openCode">, ask: OneShotAsk): Promise<string> => {
    ...
    try {
        ...
        /* NO SESSION LEFT BEHIND, the same promise the Claude Code helper keeps with `persistSession: false`, kept here by
         * hand because OpenCode has no such option. Without it every landing would file a one-turn session in the
         * runtime's own store, which is the history-list pollution claude-one-shot.ts records paying for once already. */
