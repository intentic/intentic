...
test("what a peer published is there for the next boot, keyed by its door", async () => {
    ...
    await vi.waitFor(async () => expect(JSON.parse(await readFile(peerToolsFile(historyRoot), "utf8"))).toEqual({ peers: { "webext:chrome": tools } }));

    const afterRestart = filePeerTools(historyRoot, silent);
    await vi.waitFor(() => expect(afterRestart.get("webext:chrome")).toEqual(tools));
...
test("a peer that has already answered this boot beats what the file remembers", async () => {
    ...
    await vi.waitFor(async () => expect(await readFile(peerToolsFile(historyRoot), "utf8")).toContain("run_command"));

    const afterRestart = filePeerTools(historyRoot, silent);
    afterRestart.set("hosts:laptop", { tools: [{ name: "run_command" }, { name: "screenshot" }] });
    await vi.waitFor(async () => expect(await readFile(peerToolsFile(historyRoot), "utf8")).toContain("screenshot"));
