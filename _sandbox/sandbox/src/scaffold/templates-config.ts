...
// Listing means fetching the source to read its manifest, so memoize per source#ref, the "New app" picker hits
// this each open. In-flight promises are shared; a failed fetch is evicted so a later retry re-fetches. Cache
