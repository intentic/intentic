...
const run = async (rules: Parameters<typeof outboundGuardHooks>[0], command: unknown): Promise<SyncHookJSONOutput> => {
    const matcher = outboundGuardHooks(rules).PreToolUse?.[0];
