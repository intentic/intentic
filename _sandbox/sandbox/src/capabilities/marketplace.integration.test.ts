...
test("resolves every clonable source shape onto capability configs; npm stays uninstallable", async () => {
    ...
    // Every row carries a kind and a trust, defaulted here because this registry states neither.
        ...
        plugins: [
            {
                ...
                version: "1.0.0",
                install: { url, path: "plugins/alpha" },
            },
            {
                name: "gh",
                kind: "plugin",
                trust: "listed",
                admitted: true,
                install: { url: "https://github.com/owner/gh-plugin.git", ref: "v2" },
            },
            { name: "npm-only", kind: "plugin", trust: "listed", admitted: true },
            // An exact sha pins harder than a ref when both are present.
            {
                ...
                trust: "listed",
                admitted: true,
                install: { url: "https://example.com/p.git", ref: "abc123" },
            },
            {
                name: "sub",
                kind: "plugin",
                trust: "listed",
                admitted: true,
                install: { url: "https://example.com/mono.git", path: "tools/plugin", ref: "v1" },
            },
        ],
    });
        ...
        JSON.stringify({
            ...
            plugins: [
                { name: "popular", kind: "extension", source: { source: "github", repo: "acme/popular", sha: "a".repeat(40) } },
                {
                    ...
                    securityReview: {
                        sha: "b".repeat(40),
                        url: "https://github.com/acme/checked.git",
                        policy: "team-policy",
                    ...
                    source: { source: "github", repo: "acme/checked", sha: "b".repeat(40) },
                },
                {
                    name: "evil",
                    kind: "extension",
...
test("a registry with no generated facts file resolves fine: most registries run no scanner", async () => {
    ...
    expect(marketplace.plugins).toEqual([
        { name: "internal", kind: "extension", trust: "listed", admitted: true, install: { url, path: "internal" } },
    ]);
});
