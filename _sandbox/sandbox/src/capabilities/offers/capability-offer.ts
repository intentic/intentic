...
export interface CapabilityGate {
    readonly ask: (asked: AskedCapability) => Promise<CliAnswer>;
}
...
export const createCapabilityGate = (deps: AskDeps): CapabilityGate => {
    ...
    const ask = async (asked: AskedCapability): Promise<CliAnswer> => {
        ...
        if (entry === undefined) {
            return refusalAnswer(404, "unknown_capability", `No capability entry is named "${asked.entry}": \`capabilities list\` names what exists.`);
        }
        ...
        switch (stateOf(run.conversationId, entry.id)) {
            case "parked":
                return refusalAnswer(409, "already_asked", `You already asked for ${entry.name}: that request is still up; wait for its answer.`);
            case "declined":
                return refusalAnswer(403, "declined", `The owner already skipped connecting ${entry.name} in this conversation: continue without it.`);
            case undefined:
                break;
        }
        ...
        const request = await raiseRequest(deps, run, {
            ...
            approves: (reply) => reply.connect,
            signal: asked.signal,
            deadlineMs: deps.deadlineMs ?? OFFER_DEADLINE_MS,
        });
        if (request.decision === "unanswered") {
