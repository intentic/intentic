...
const fake = (over: Partial<AskDeps> = {}): Fake => {
    ...
    return { deps, frames, observed, awaited, manifest, states };
};
...
it("a yes parks the call on the setup, and the connection coming live answers it connected", async () => {
    const { deps, frames, observed, awaited, manifest, states } = fake();
    ...
    // The owner's devices hear about it once, as they would about a card the turn raised itself.
    expect(awaited).toEqual([{ conversationId: "conv-1", kind: "capability_offer" }]);
});
