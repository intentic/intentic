...
export interface PrepushCheck {
    ...
    // the suite finishes, so the caller's first poll never sees a stale `idle`.
...
export const createPrepushCheck = (services: PrepushDeps): PrepushCheck => {
    ...
    return {
        ...
        state: async () => {
            // No rule means off, whatever the last run concluded; a stale result must not keep gating a push.
