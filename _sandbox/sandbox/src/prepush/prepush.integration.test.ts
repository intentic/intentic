...
test("a running check names its terminal and carries no output of its own", async () => {
    ...
    await waitFor(async () => expect((await check.state()).session).toBe(CHECKS_SESSION), SETTLES);
...
test("a sandbox without the tmux wrapper names no terminal", async () => {
    ...
    const check = createPrepushCheck(services);
    await check.run();
    await waitFor(async () => expect((await check.state()).status).toBe("passed"), SETTLES);
...
test("a non-zero exit is a failed result carrying the output", async () => {
    ...
    const check = createPrepushCheck(services);
    await check.run();
    await waitFor(async () => expect((await check.state()).status).toBe("failed"), SETTLES);
    const state = await check.state();
...
test("a second run while one is going starts nothing", async () => {
    ...
    await waitFor(async () => expect((await check.state()).status).toBe("running"), SETTLES);
    await check.run();
    await waitFor(async () => expect((await check.state()).status).toBe("passed"), SETTLES);
    expect(runs()).toBe(1);
});
...
test("a cancelled run is cancelled, not failed", async () => {
    ...
    await waitFor(async () => expect((await check.state()).status).toBe("running"), SETTLES);
    check.cancel();
    await waitFor(async () => expect((await check.state()).status).toBe("cancelled"), SETTLES);
});
...
test("a red verdict notifies devices; a pass and a cancel say nothing", async () => {
    ...
    await redCheck.run();
    await waitFor(async () => expect((await redCheck.state()).status).toBe("failed"), SETTLES);
    ...
    await waitFor(async () => expect((await greenCheck.state()).status).toBe("passed"), SETTLES);
    ...
    await waitFor(async () => expect((await stoppedCheck.state()).status).toBe("running"), SETTLES);
    stoppedCheck.cancel();
    await waitFor(async () => expect((await stoppedCheck.state()).status).toBe("cancelled"), SETTLES);
...
test("a command the shell cannot find is a failure whose output says so", async () => {
    ...
    const check = createPrepushCheck(services);
    await check.run();
    await waitFor(async () => expect((await check.state()).status).not.toBe("running"), SETTLES);
    const state = await check.state();
