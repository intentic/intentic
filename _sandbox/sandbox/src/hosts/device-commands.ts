...
/* THIS PRODUCT'S OWN CLI, RUN FROM A BUTTON, on a device the user connected.
 *
 ...
 * own answer rather than being cut off mid-sentence from here (the same pairing device-reports.ts uses). */
...
interface DeviceCommandSpec {
    ...
    /* Whether this one REFUSES to run fleet-wide. Bare, the agent's CLI acts on every sandbox that device
 ...
 * the id it is looking at. Bare, it acts on every sandbox that device pairs — which is what these commands
 * mean in a terminal, and what the MACHINE's own switches on the Devices tab send, because "stop syncing on
...
/* Run one named action on one connected device.
 *
