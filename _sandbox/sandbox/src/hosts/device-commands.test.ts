...
// id scopes it to one, so a button on one row never reaches a colleague's pairing on the same device.
 ...
 * device pairs, which is the honest "turn it off entirely" for the reversible switches and a trap here:
