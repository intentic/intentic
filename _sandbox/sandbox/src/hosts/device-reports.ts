...
/* The ceiling on watching an agent update, which is a different shape of wait: a ~95 MB download on a slow line
 * is minutes, and then the connection carrying the narration goes down on purpose. Well above the download and
 * well below the hour above it, because there is nothing to keep waiting FOR once the loop restarts — the
 * device's reported version is the answer, and the view is already polling for it. */
 ...
 * somebody who just pressed Stop is owed (the same rule manageDeviceSandbox states from its own `finally`). */
...
export const reportFrom = (text: string): DeviceReport | undefined => {
    for (const line of text.split(/\r?\n/).toReversed()) {
        ...
        // device links — is the daemon's own knowledge already, so only this half is read).
...
// Exported for device-commands.ts, which runs the user's own CLI on the same socket and reads its answer the
...
const pull = async (services: Services, id: string): Promise<PullResult> => {
    ...
    /* The report's own agent block is left exactly as the device stated it. What the socket announced at connect
     * used to be folded in here as a second `agents.host` entry, which was the same number twice: one resident
     * process stamps its `MACHINE_VERSION` into the pidfile (`agent.build`) and sends it in the hello frame. It
     * rides the ROW instead (`Device.agentVersion`, filled in mergeDevices), where it is the only version a
     * device with no report at all can still offer. */
 ...
 *
 * This is the whole of the Devices view's latency fix. The route used to be a live fan-out: a reader waited on
 * a round trip to every one of their laptops, and the measured cost of that was a p50 of ~9.8s with a tail past a
 * minute. Nothing about it needed to be synchronous. A reading is a snapshot of somebody's device that carries
...
/* WHICH OS, in the one vocabulary the view renders, the capability cards' own slugs (see the `devices`
...
export const mergeDevices = (
    ...
    /* WHICH ROW A DEVICE IS, and there are two ways to know it, of very different strengths.
     *
     ...
     * here, not an edge one. Left unjoined it forked into a second row for a device already on screen: same
     * name, no report, nothing to show. The two then disagreed about one box, and each way of disagreeing was a
     * bug somebody hit. The sync row, seeing no device door, offered "Connect this device" for a device
     ...
     * chose for the same box, typed identically, landing on a row that is about to say the device is asleep
     * and offer nothing.
     *
     ...
     * do the silent ones claim what is left. Without that ordering an ASLEEP device named like the enrollment
     ...
     * always available to tell two rows about different devices apart. */
    ...
    for (const { host, result } of [...answered, ...silent]) {
        ...
        if (existing !== undefined) {
             ...
             * volunteered, and `online: false` is what says the device half is unreachable. The gap goes only
             ...
             * why) from the device door's own state instead (manageBlock). */
    ...
    // An enrolled machine with no report says so, rather than rendering as a device that has no folders and no
 ...
 * http.request that contains it: `devices.read` is this reconciliation, `devices.pull` is one round trip to
...
/* One management action on one machine's sandbox, the Devices view's buttons, relayed to the machine and
...
export async function* manageDeviceSandbox(services: Services, id: string, input: DeviceSandboxFlow): AsyncGenerator<DeviceFlowLine> {
     ...
     * refusal is here, before a container exists on somebody's device with no way home. */
...
/* UPDATING OR RESTARTING THE AGENT ON ONE DEVICE, relayed the same way — and the one flow that expects its own
 * transport to die.
 *
 * Both ops stop the resident process on that machine, and that process is what holds this socket, so the stream
 * ends without a terminal frame in the ORDINARY case. Nothing here dresses that up as an error: the lines the
 * device managed to send are the lines, and the confirmation is its agent version moving on the next read (the
 * work itself is detached over there before the loop goes down, so it finishes regardless — see the agent's
 * device/tools/agent.ts for why it must be).
 *
 * An agent too old to serve the procedure at all is a legitimate answer too. oRPC rejects the call, this
 * converts it into the terminal `error` line the browser already renders, and it names the command that does
 * the job by hand — which is exactly what the view printed before this route existed. */
export async function* runDeviceAgentFlow(services: Services, id: string, input: DeviceAgentFlow): AsyncGenerator<DeviceFlowLine> {
    ...
    try {
        ...
        /* The socket dying mid-flow is the SUCCESS path here, so it must not surface as a failure: the loop this
         * very call asked to be restarted is the thing that was carrying it. Distinguishing "the agent went down
         * as asked" from "this device cannot do this" is not something either side can do from here, which is
         * precisely why the view confirms by version rather than by outcome. What IS worth passing through is a
         * refusal the device managed to answer with — an old agent with no such procedure, or its "Run commands"
         * switch turned off — because each names something the reader can act on. */
        ...
        // Whatever happened, the device's agent may be a different build now, so the next read must ask rather
        // than serve what the TTL remembers: the version IS this flow's answer.
        pulled.delete(id);
    }
}
