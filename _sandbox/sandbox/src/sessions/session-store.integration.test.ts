...
test("links every conversation-owned ~/.claude store to the workspace, creating both ends", async () => {
    ...
    for (const name of ["projects", "plans", "backups", "tasks", "sessions", "session-env", "shell-snapshots", "todos"]) {
        expect(await readlink(join(home, ".claude", name))).toBe(join(work, ".intentic", "records", "sessions", "claude", name));
        expect((await lstat(join(work, ".intentic", "records", "sessions", "claude", name))).isDirectory()).toBe(true);
    }
});
...
test("a second run (daemon restart in the same container) is a no-op", async () => {
    ...
    await linkClaudeState(work, home);
    await linkClaudeState(work, home);
    expect(await readlink(join(home, ".claude", "projects"))).toBe(join(work, ".intentic", "records", "sessions", "claude", "projects"));
});
...
test("a real directory (a dev-host run) throws, is left intact, and does not block the other links", async () => {
    ...
    expect(await readlink(join(home, ".claude", "plans"))).toBe(join(work, ".intentic", "records", "sessions", "claude", "plans"));
});
