...
// The core-door half of "speak as the agent". A Front Desk has no gateway extension and never will, so without this
...
test("a Front Desk origin queues the reply for its own visitor and reports it delivered", async () => {
    const { outbox, services } = harness();
    const outcome = await deliverToListenerChannel(services, { automationId: "desk", provider: "webchat", channelId: "visitor-1" }, "answered by a person");

    expect(outcome).toBe("delivered");
    expect((await outbox.since("webchat:desk:visitor-1", 0, Date.now())).replies).toMatchObject([{ text: "answered by a person" }]);
    // Another visitor's thread is untouched: the channel id is the whole address.
    expect((await outbox.since("webchat:desk:visitor-2", 0, Date.now())).replies).toEqual([]);
});
...
test("an origin with no channel has nowhere to deliver, and says so rather than reaching for a gateway", async () => {
    ...
    expect(await deliverToListenerChannel(services, { automationId: "desk", provider: "webchat" }, "hello")).toBe("no-gateway");
});
