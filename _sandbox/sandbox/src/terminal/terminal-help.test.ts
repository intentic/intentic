...
test("killing the session settles its open ask as not-helped", async () => {
    const { id, wait } = cards.create("terminal_help", { kind: "terminal_help", requestId: "", helped: false, note: "aborted" });
    // The raiser binds how its card settles unhelped, as the terminal tool does.
    raiseTerminalHelp("agent-he1p0002", { requestId: id, message: "touch the security key", requestedAt: 2 }, (note) =>
        cards.resolve({ kind: "terminal_help", requestId: id, helped: false, note }),
    );
...
test("a turn aborting under the ask settles the waiter, and clearing takes the banner down", async () => {
    ...
    const { id, wait } = cards.create("terminal_help", {
        kind: "terminal_help",
        requestId: "",
        helped: false,
        note: "the turn ended before anyone could help",
    });
    raiseTerminalHelp("agent-he1p0004", { requestId: id, message: "confirm the prompt", requestedAt: 3 }, (note) =>
        cards.resolve({ kind: "terminal_help", requestId: id, helped: false, note }),
    );
