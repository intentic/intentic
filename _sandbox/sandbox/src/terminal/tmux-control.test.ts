...
// Parses tmux 3.3a control-mode transcripts fed in the chunk shapes a real pipe delivers (whole lines, half lines,
// several at once); a misread means wrong bytes or a hung reply.
...
test("%output: octal escapes become bytes, everything else passes as the byte it is", () => {
    // ESC, backslash, newline, and UTF-8 for "é"; tmux passes bytes >= 0x80 through unescaped.
...
test("a location reads back the session it is in, which is what keeps a tab off another session's window", () => {
    ...
    // Empty when the target is gone or the reply block was empty; never a partial location.
    expect(parseLocation("")).toBeUndefined();
    expect(parseLocation("can't find window: @9")).toBeUndefined();
    // Ids are sigil-checked, so a field landing in the wrong position can't read as a valid location.
    expect(parseLocation("%7 @2 $3")).toBeUndefined();
});

// Pane state as tmux 3.3a printed it for a fresh zsh in a 100x30 pane, paired with the bytes that put an empty xterm
// into the same state.
...
test("a normal screen: reset, the lines CRLF-joined and unterminated, the cursor, then the modes", () => {
    ...
    // RIS first, clearing any earlier screen and scrollback before the replay refills them.
    expect(bytes.startsWith("\x1bc$ ls\r\na  b\r\n$ ")).toBe(true);
    // No trailing CRLF: the cursor stays on the last captured row; the move targets row 3, col 3.
    expect(bytes).toContain("$ \x1b[3;3H");
    // A full-height scroll region is the default and unstated; zsh's DECCKM/keypad flags are stated; bracketed paste is
    // assumed for a shell on the normal screen.
