 ...
 * refreshes.
 *
 * That is not hypothetical. The agent's memory notes live at `.intentic/claude/projects/<slug>/memory/**`, under
 * the `.intentic/claude` prefix the watcher excludes wholesale (session transcripts are rewritten on every
 * streamed token). Declaring `{ path: ".intentic/claude/...", invalidates: ["memory"] }` would read as a fix and
 ...
 * watcher descend `.intentic/claude` → `projects` → all 116 project slugs, measured as +119 watched directories
