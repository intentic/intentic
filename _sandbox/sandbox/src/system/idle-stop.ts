 ...
 * the verdict and the act: once every minute it asks four questions, and only when all four have answered
...
export const startIdleStop = (
    ...
    const check = async (): Promise<void> => {
        ...
        if (probes.connected() > 0 || probes.turns() > 0 || probes.delegates() > 0) {
            quietSince = now;
            return;
        }
    ...
    const timer = setInterval(() => void check().catch((error: unknown) => args.logger.warn({ err: error }, "idle-stop check failed")), CHECK_INTERVAL_MS);
