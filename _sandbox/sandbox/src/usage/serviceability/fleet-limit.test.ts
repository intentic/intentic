...
test("counts an account on the pools the model spends, and names the pool that is out", () => {
    ...
    expect(fleetLimit(fleet, { id: "claude-opus-4-6-thinking" })).toEqual({
        pool: "Claude and GPT models",
        spent: 1,
        withHeadroom: 1,
        roomMeasuredAt: 1_000,
    });
...
test("an undivided plan gates every model on every window, and has no pool to name", () => {
    const fleet = [
        reading(
            "one",
            window({ kind: "five_hour", utilization: 100, resetsAt: 1_000 }),
            window({ kind: "seven_day", utilization: 12, resetsAt: 8_000 }),
        ),
    ];
...
test("a Claude account's spent Opus slice does not count against a Haiku call", () => {
    const fleet = [
        reading(
            ...
            window({ kind: "model:Opus", label: "Opus", utilization: 100, resetsAt: 4_000, gates: { models: ["Opus"] } }),
        ),
    ];
...
test("with no model named, every pool that gates anything counts and one gating nothing never does", () => {
    const fleet = [
        reading("a", window({ kind: "seven_day", utilization: 20 }), window({ kind: "code-review:five_hour", utilization: 100, gates: "none" })),
    ];
