...
// A turn's remote MCP endpoints, and the release its loop calls when the turn ends: the extension cards' mount bearer
// stops reaching them then (extension-mcp.ts).
...
export const turnToolsOf = async (
    services: Pick<Services, "tools" | "config" | "hostBridgeToken" | "webextBridgeToken" | "extensionMcpMounts"> & ExtensionHost,
    granted: readonly Capability[],
    conversationId?: string,
): Promise<TurnRemoteTools> => {
    const extension = await extensionMcpToolsOf(services, granted, services.config.sandbox.port, conversationId);
    return {
        tools: [
            ...services.tools,
            ...mcpToolsOf(granted),
