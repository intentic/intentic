...
import { createCapabilityAskRoutes } from "./capabilities/offers/ask.routes.js";
...
import { reachPosture } from "./system/listeners/reach-posture.js";
import { profileTraits } from "./system/boot/profile.js";
import { rawRouteServer } from "./http/raw-route-server.js";
import { logContext } from "./logger.js";
import { stateStatus } from "./store/evolution/state-convergence.js";
...
export const createApp = (services: Services): Hono<AppEnv> => {
    ...
    serve("GET /health", (c) =>
        c.json({
            ...
            // Whether an update's state conversions are still uncommitted (store/evolution/state-convergence.ts); the host rolls
    ...
    serve(
        "ALL /mcp/{mount}",
        createTurnMountRoute(services.turnMounts, {
