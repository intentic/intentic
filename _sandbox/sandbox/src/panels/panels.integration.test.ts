...
test("a listener is named by the package that bound it; one at the repo root has no name to give", () => {
    expect(listenerDir(listener(47145, "/work/intentic/_editor/web"), "/work", "intentic")).toBe("_editor/web");
...
test("a package's sidecar sockets collapse into the one server the app is on", () => {
    expect(
        oneServerPerDir([
            ...
            { url: `http://localhost:47146`, dir: `_site/demo` },
            { url: `http://localhost:47180`, dir: `_site/demo` }, // the demo's own sidecar
            { url: `http://localhost:47199`, dir: `_editor/web` }, // vite's HMR channel
        ]),
    ).toEqual([
        { url: `http://localhost:4321`, dir: `_site/site` },
        { url: `https://localhost:47145`, dir: `_editor/web` },
