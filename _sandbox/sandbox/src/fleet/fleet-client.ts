...
export const fleetWhoami = (config: Config, token: string): Promise<RelayedAnswer> =>
...
export const fleetList = (config: Config, token: string): Promise<RelayedAnswer> =>
...
export const fleetProvision = (config: Config, token: string, ask: ProvisionAsk): Promise<RelayedAnswer> =>
    callPlatform(config, {
        method: "POST",
