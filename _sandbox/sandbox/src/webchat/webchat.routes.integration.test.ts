...
test("an allowed visitor message wakes the agent and streams the reply back as SSE", async () => {
    ...
    // Arrives sealed in an <untrusted-content> envelope and flagged to the guard layer independently of it; both halves
    // are asserted here, the public entry point.
    const sealed = /<untrusted-content source="webchat" id="([0-9a-f]{16})">\n([\s\S]*)\n<\/untrusted-content id="\1">/.exec(turns[0]?.prompt ?? "");
    // The visitor's payload rides in one envelope: their text and their chosen name.
...
// A skipped run still gets the same closed-stream treatment as an errored wake.
...
test("a Front Desk with no configured ceiling still stops at the default", async () => {
    ...
    for (let sent = 0; sent < WEBCHAT_DAILY_MAX_DEFAULT; sent += 1) {
        // One conversation per message, so the per-conversation ceiling isn't what stops it.
...
// The default is a floor under the unconfigured case, not a cap on explicit intent.
...
test("with the proof-of-work check on, a first message without an answer is refused and a solved one gets through", async () => {
    ...
    // Spent once per thread: the follow-up carries no PoW answer and is admitted anyway.
...
test("config resolves every default and never emits a secret", async () => {
    ...
    // A greeting nobody set still resolves to something; the widget carries no fallback of its own.
...
test("a bot check configured without its keys reports off rather than becoming a gate nobody can pass", async () => {
    ...
    // The message route agrees too: the same resolution decides both.
...
test("sign-in-only refuses an anonymous message with 401 so the widget knows to re-open its gate", async () => {
    ...
    // An unverifiable token refuses the same way: an expired-vs-forged breakdown only helps someone probing.
    expect((await post(app, "wc-auth", { conversationId: "v", content: "hi", idToken: "not-a-jwt" })).status).toBe(401);
    expect(turns).toEqual([]);
});

// Install probes: telling nobody wrote yet from the snippet never landing.
