...
// The picker catalog for one endpoint-minting capability (`endpoint`, or a `localmodel` read through its
// derived loopback config). Unlike the four fixed provider routes this one resolves its subject first: the id
// in the path names a capability, and an id that names none is a NOT_FOUND rather than an empty list, "this
// endpoint was removed" and "this endpoint publishes nothing" are different problems with different fixes, and
// a blank catalog cannot tell them apart.
