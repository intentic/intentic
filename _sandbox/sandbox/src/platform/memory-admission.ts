...
/* THE STALL BACKSTOP, for a box that has swap under it.
 *
 * With swap disabled — which is now every sandbox the run contract creates (@intentic/sandbox-run) — a box that
 * runs out fails immediately and loudly, so `freeBytes` is the signal that matters and PSI barely moves before
 * the kill. But a sandbox created BEFORE that change still carries its old allowance, and the hosted shape
 * carries no cap at all, and on those the failure is the slow one: the box does not run out, it grinds, and
 * `freeBytes` can look survivable while nothing in the container is actually progressing. `full avg10` is what
 * names that state. Twenty percent is well clear of the noise a healthy box makes (0, essentially always) and
 * far below the eighty-plus a genuinely stalled one sits at, so it catches the grind early without firing on a
 * momentary reclaim spike. */
