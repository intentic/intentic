...
const SLOW_MS: Readonly<Record<string, number>> = {
    ...
    "http.request": 1_000,
    /* One round trip to one of the user's own devices, over its WebSocket, to read what it is running. The
     ...
     * deadline in device-reports.ts cuts it off shortly after.
     *
     * It is off the request path (the Devices route serves its last reading and refreshes behind it), so a slow
     * row here is a fact about a MACHINE, and reading it next to `devices.read` is what tells the two apart. */
    "devices.pull": 4_000,
