...
import { createVerificationLedger, type VerificationLedger } from "../agent/verification/agent-verification.js";
...
// The one moment that can send work back: a Stop that says something keeps the turn going. Conditions are read here,
// not at planning time, since nothing knows what a turn touched until it runs.
...
// `verification` answers which code paths the turn edited; `view`, whether it looked after the last surface edit.
interface Ledgers {
...
const BUILTINS: Record<RuleBuiltin, (ledgers: Ledgers) => Promise<string | undefined>> = {
    "verify-ui-edits": async (ledgers) => verifyUiEditsMessage(ledgers.view),
...
const contributionOf = async (rule: Rule, deps: TurnEndingDeps, ledgers: Ledgers): Promise<string | undefined> => {
    if (rule.action.kind === "builtin") {
        return BUILTINS[rule.action.name](ledgers);
    }
...
export const turnEndingHooks = (rules: readonly Rule[], deps: TurnEndingDeps = {}): Partial<Record<HookEvent, HookCallbackMatcher[]>> => {
    ...
    const ledgers: Ledgers = { verification: createVerificationLedger(), view: createViewLedger() };
    let followUps = 0;
    ...
    return {
        PostToolUse: [
            ...
            },
            {
                ...
                hooks: [
                    async (input) => {
                        if (input.hook_event_name === "PostToolUse" && bashCommand(input.tool_input) !== undefined) {
                            commands += 1;
                        }
                        return {};
                    },
