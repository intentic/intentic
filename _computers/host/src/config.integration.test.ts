 ...
 * it from the first. That is not an exotic path: the last step of onboarding runs `intentic-host setup`, so
...
beforeAll(async () => {
    home = await mkdtemp(join(tmpdir(), "intentic-host-config-"));
