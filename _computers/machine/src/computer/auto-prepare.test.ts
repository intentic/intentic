...
const box = (slug: string, running = true): MachineSandbox => ({
    slug,
...
test("a prepare that THROWS (no ic on this machine) is a failure with backoff, not an unhandled rejection", async () => {
    const state = newState();
    const lines: string[] = [];
    await runTick(state, [box("work")], () => Promise.reject(new Error("This computer has no `ic` command")), (line) => lines.push(line), new Set());
    expect(lines).toEqual(["auto-prepare work: failed (attempt 1, retrying after 1 tick(s)) — This computer has no `ic` command"]);
