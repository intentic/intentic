...
// The include lines that are OURS to add and remove: the current relative one, and the spellings earlier
// builds wrote (the standalone sync agent's relative name, and before that an absolute path). The old ones are
// stripped rather than left alone because they are worse than dead weight: their files carry Host blocks for
// the SAME aliases this agent writes, so whichever include ssh reads first wins, and a stale file pins every
// alias to whatever hostname it held the day that agent was replaced.
