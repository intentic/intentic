...
// The published asset for THIS machine, the same URL, built the same way, that computer.sh/sync.sh resolve. `latest` rather
// than a pinned version: this command's whole promise is "put me on the current one".
export const assetUrl = (): string =>
    `https://github.com/intentic/intentic/releases/latest/download/intentic-machine-${osToken()}-${archToken()}${exe}`;
...
export const runUpgrade = async (exec: UpgradeExec, url: string, installed: string, force: boolean, log: Log): Promise<UpgradeOutcome> => {
    ...
    if (installed === DEV_VERSION && !force) {
        ...
        return {
            kind: "current",
            version: installed,
            note: `this agent was built from source, not installed from a release. \`intentic-machine upgrade --force\` replaces it with the published ${candidate}.`,
        };
    }
