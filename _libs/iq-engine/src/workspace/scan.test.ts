...
test("sweep admits tracked-looking files and is path-sorted", () => {
    expect(paths()).toContain("repositories/alpha/src/widget.ts");
    expect(paths()).toContain("repositories/beta/app.py");
...
test("sweep enforces .gitignore + junk dirs (incl. .git) by default and always self-excludes the index dir", async () => {
    expect(paths()).not.toContain("repositories/alpha/dist/decoy.js"); // .gitignore layer
    ...
    expect(fullPaths).toContain("repositories/alpha/dist/decoy.js"); // --ignored lifts .gitignore + junk dirs…
...
test("sweep tags files with their enclosing git repo", () => {
    const widget = entries.find((entry) => entry.path === "repositories/alpha/src/widget.ts");
    expect(widget?.repo).toBe("repositories/alpha");
...
test("filterScope narrows by path, lang, glob, and file class", () => {
    expect(filterScope(entries, { paths: ["repositories/beta"] }).map((entry) => entry.path)).toEqual(["repositories/beta/app.py"]);
    expect(filterScope(entries, { langs: ["python"] }).map((entry) => entry.path)).toEqual(["repositories/beta/app.py"]);
    ...
    expect(filterScope(entries, { only: "tests" }).map((entry) => entry.path)).toEqual(["repositories/alpha/src/widget.spec.ts"]);
    expect(filterScope(entries, { repo: "alpha" }).every((entry) => entry.path.startsWith("repositories/alpha/"))).toBe(true);
});
