...
// A "start from" suggestion in the new-automation dialog, shown only when the matching capability provider is
...
// scheduled-poll recipes that lean on the agent's CLI capability instead.
export interface AutomationRecipe {
    // Matches CapabilitySummary.config.provider — the recipe shows only when that capability is enabled. Absent
    // ⇒ always shown (a recipe that needs no capability, e.g. publishing drafts through whatever skills exist).
    readonly provider?: string;
    readonly title: string;
    // Simple-icons slug, same convention as CapabilityCatalogEntry.logo.
    readonly logo?: string;
