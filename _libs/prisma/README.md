...
The **database layer** — the Prisma schema, the generated client, and the migrations. Owns the Postgres data model and exposes the typed `PrismaClient` that [`@intentic-app/api`](../../_apps/api) imports. Prisma 7 with the `prisma-client` generator (output to `./generated`).
