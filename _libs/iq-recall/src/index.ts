...
export const createRecall = (options: RecallOptions): Recall => {
    ...
    // Mirrors iq-engine's IQ_DIR ("<root>/.intentic/iq") without dragging its heavy dependency tree in for
    // one constant — recall.db sits next to index.db, inside the dir iq already excludes from search.
    const dbPath = options.dbPath ?? join(options.root, ".intentic/iq/recall.db");
