...
-- BM25 over typed prompts and session titles. External-content: rows live in turns/sessions; the triggers
-- keep the FTS index in sync (they fire on FK-cascade deletes too). tokenchars keeps snake_case/$identifiers
-- whole — same setup as iq-engine's chunks_fts.
CREATE VIRTUAL TABLE IF NOT EXISTS turns_fts USING fts5(prompt, content='turns', content_rowid='id', tokenize="unicode61 tokenchars '_$'");
CREATE TRIGGER IF NOT EXISTS turns_fts_ai AFTER INSERT ON turns BEGIN
    INSERT INTO turns_fts(rowid, prompt) VALUES (new.id, new.prompt);
END;
CREATE TRIGGER IF NOT EXISTS turns_fts_ad AFTER DELETE ON turns BEGIN
    INSERT INTO turns_fts(turns_fts, rowid, prompt) VALUES ('delete', old.id, old.prompt);
END;
