...
test("graph: a natural-language query returns related definition anchors; off drops them", async () => {
    ...
    // The fixture is tiny: related lines appear only when a top hit sits inside a symbol.
...
test("pack: top natural-language groups carry the enclosing symbol body as contiguous lines; off keeps sparse hits", async () => {
    ...
    // Contiguous lines are the packed slice; anchors beyond it may follow as pointers, so only the prefix is checked
    // here.
...
// Ordering and content are deterministic; raw scores carry recency-boost timestamp jitter.
