...
export const FIX_DEPS_AUTOMATION = {
    ...
    prompt:
        ...
        "Re-run only what failed to see it first-hand (the failing package's tests or typecheck, not the whole check command), then fix the ROOT CAUSE. That usually means updating call " +
        ...
        "Finish by re-running what failed and reporting what was broken, what you changed, and whether it passes now; the " +
        "daemon runs the whole check again once your fix lands.",
} as const;
