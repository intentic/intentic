...
// The command a request is holding, as data rather than prose. `spans` is carried from the classifier's own match (never
...
export const ProgramAskSchema = z.object({
    ...
    truncated: z
        .boolean()
        .describe(
            "Whether this is an excerpt of a longer program, so the request can say so instead of ending mid-word. An excerpt always carries the flagged fragment: the beginning, then a window around the fragment, with any skipped middle written into the text as a bracketed count.",
        ),
    spans: z
...
export const PermissionAskSchema = z.object({
    ...
    // Why the prompt fired ('rule' | 'mode' | 'classifier' | …), shown as the request's muted subline.
...
// One missing capability asked for; `request`/`name` are resolved by the daemon from the catalog it validates against. The
// model contributes only `why`, so the request can't misrepresent what's being connected.
export const CapabilityOfferSchema = z.object({
    // The catalog request being asked for, and how the catalog itself titles it ("Notion", "GitHub", "Docker").
    entry: z.string().describe("Which catalog entry is being asked for."),
    name: z.string().describe("What it is called, as the catalogue titles it rather than as the agent named it."),
    // The agent's one-line case for connecting it, the only prose on the request that is the model's.
    why: z.string().optional().describe("The agent's case for connecting it, and the only words on this request that are the agent's."),
});
...
export const PaymentOfferSchema = z.object({
    ...
    // The agent's one-line case for paying, the only prose on the request that is the model's.
    why: z.string().optional().describe("The agent's case for paying, and the only words on this request that are the agent's."),
});
...
// would spend it. `approvers` are shown since the request addresses a named list, not the owner.
export const CredentialOfferSchema = z.object({
    ...
    // The agent's one-line case, the only prose on the request that is the model's.
    why: z.string().optional().describe("The agent's case for using it, and the only words on this request that are the agent's."),
    approvers: z.array(z.string()).describe("Who may release it. A click from anyone else is refused and leaves the request standing."),
...
// ACP-aligned tool taxonomy (Agent Client Protocol's ToolKind, verbatim): what a tool call does, driving the request icon
...
// The document a parked request is asking about, carried by value: the bytes are already in hand, and a restored
...
export const permissionRequest = { requestId: REQUEST_ID };
...
export const browserHelpRequest = {
    requestId: z.string(),
    session: z.string(),
    account: z.string(),
    message: z.string(),
...
// the terminal panel, same division as the browser request.
export const terminalHelpRequest = {
    requestId: z.string(),
    session: z.string(),
    message: z.string(),
};
...
export const credentialOfferRequest = { requestId: z.string(), offer: CredentialOfferSchema };

// How an accepted capability offer ended, shared by the frame that reports it and the record that keeps it, so a
// reopened receipt matches the live request.
...
// The three requests a turn journal can restore verbatim after a daemon restart. `browser_help`/`terminal_help` are
...
export const PermissionRequestSchema = PermissionAskSchema.extend({
    kind: z.literal("permission").describe("The agent wants to use a tool it needs permission for."),
    ...permissionRequest,
});
export const ParkedRequestSchema = z.discriminatedUnion("kind", [PlanRequestSchema, QuestionRequestSchema, PermissionRequestSchema]);
export type ParkedRequest = z.infer<typeof ParkedRequestSchema>;
