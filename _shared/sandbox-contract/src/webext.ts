...
// `index.ts` reaches every schema and contract in the repo, and the extension's bundle is uploaded to a store, where
// ~400 kB of the daemon's unrelated wire surface is both dead weight and something a reviewer has to account for.
// Nothing here may import the barrel, or the saving is silently undone — `pnpm --filter @intentic/webext build`
