...
for (const subpath of SUBPATHS) {
    walk(join(OUT_KIT, `${subpath}.d.ts`));
}
...
for (const name of [`index.d.ts`, ...SUBPATHS.map((subpath) => `${subpath}.d.ts`)]) {
...
// One bridge per narrowed entry point, each holding the names index.ts re-exports from that file, so a subpath and
...
const subpathCounts = SUBPATHS.map((subpath) => {
    ...
    writeFileSync(
        ...
        [
            `// Generated by scripts/build.mjs: do not edit.`,
            `// The kit's ${subpath} entry point, off the same host bridge as ./index.js.`,
            `const host = globalThis.__intenticHost;`,
            `if (host === undefined) {`,
    ...
    return `dist/${subpath}.js: ${names.length}`;
});

rmSync(STAGING, { recursive: true, force: true });
log([`dist/index.js: ${bridgeNames.length} exports`, ...subpathCounts].join(` · `));
