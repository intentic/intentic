...
// Decodes an SSE response's output items; reading the wire text directly risks a wrong backslash count passing as
// right.
...
test("an execScript step escapes the command into valid JavaScript", async () => {
    ...
    try {
        ...
        // Also checks the escaped source actually parses as JavaScript.
...
test("the chat-completions surface streams nameless chunks and terminates with [DONE]", async () => {
    ...
    try {
        ...
        // Same reader works across dialects: a scenario asserts one way about either.
...
test("systemInstructions and hasTool answer the same question on both model surfaces", () => {
    ...
    // user_input isn't request_user_input; a suffix match without the dot boundary would say it was.
...
test("the tool reader also sees the namespaced additional_tools item that `codex exec` sends", () => {
    ...
    // No top-level field here: the prompt lives in a developer message instead.
