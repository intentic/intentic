#!/usr/bin/env node
/* HOW BIG A LIST'S ROWS ARE IS NOT A DECISION EACH ROW GETS TO MAKE, AND THIS IS THE GATE THAT KEEPS IT.
 *
 * <Row>, <DisclosureRow> and <SkeletonRows> draw from one table of three tiers (_editor/ui/src/components/row.ts):
 * `comfortable` for settings rows, `compact` for record lists, `dense` for navigator rails. The table was right.
 * What was wrong is that every one of those components defaulted to `comfortable` PER CALL SITE, so a record list
 * was compact only while each of its rows, its loading outline and each hand-written line between them
 * independently remembered to say so. They did not:
 *
 *   · The extensions list was the one <DisclosureRow> in the app that never passed `density`, so it drew
 *     settings-sized rows one tab along from the compact secrets and personas lists. Its own header comment
 *     described the compact row it believed it was.
 *   · The personas, payouts and services outlines promised comfortable rows and landed compact ones, so those
 *     lists visibly shrank as they arrived — the payouts one under a comment stating that could not happen.
 *   · A row's lead mark was a number typed at the call site: 22 on three lists, 20 on two, 24 on a sixth and 32
 *     on a seventh, every one of them meaning "a mark on a record row".
 *   · 58 lines that live on a group's surface without being rows — an empty state, an explanatory sentence, an
 *     "add one", a form — picked their own padding, in six spellings, four of which match no tier at all.
 *
 * The tier is now the <RowGroup>'s, published to everything on its surface. What is left to guard is that
 * nobody starts answering the question locally again, which is exactly what this checks:
 *
 ...
 *   3. A LITERAL `:size` ON A MARK INSIDE `#lead`. <Row> and <DisclosureRow> hand the tier's mark size to that
 ...
 *   4. A ROW COMPONENT USED OUTSIDE A <RowGroup>. A `*Row.vue` whose own root takes no `density` is relying on
 ...
 *
 * Deliberately NOT checked: which tier a given list picked. `compact` for a record list and `comfortable` for a
 * settings list is a judgement about what the list IS, and a script that guessed would only teach people to
 * shape lists like whatever it measured. What it enforces is that the answer is given ONCE, in the one place
 * every row on the surface can read it. */
...
for (const path of tracked) {
    ...
    while ((match = TAG.exec(scan)) !== null) {
        ...
        // ── 3 · a mark sized by hand where the tier is handing the size out
        ...
        // ── 4 · where every non-native component is used, for the check below
        if (/^[A-Z]/u.test(name)) {
...
// ── 4 · a row that expects a group's tier, mounted where there is no group to read it from
