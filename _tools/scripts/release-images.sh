...
AMD64_REF="$VERSION-amd64" ARM64_REF="$VERSION-arm64" bash "$DIR/merge-image-manifests.sh" sandbox "$VERSION" stable
# The core profile's halves, published by the same jobs under core- prefixed tags (publish-images.sh TAG_PREFIX).
AMD64_REF="core-$VERSION-amd64" ARM64_REF="core-$VERSION-arm64" bash "$DIR/merge-image-manifests.sh" sandbox "core-$VERSION" core-stable
