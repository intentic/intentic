...
// What reportsAt() builds out of the snapshot's run, assembled here the same way so the test exercises the real
...
test("a finding the snapshot printed too is standing, and the turn is told so rather than charged", () => {
    ...
    const [judged] = judgeAgainstBase(live, atBase([verdict("paths", [DEAD_KEY])], true));
...
test("a standing finding that moved down the file is the same finding", () => {
    ...
    const [judged] = judgeAgainstBase([verdict("paths", [moved])], atBase([verdict("paths", [LITERAL])], true));
    assert.deepEqual(judged.added, []);
});
...
test("a check the snapshot could not run charges the turn with nothing, and still reports what was found", () => {
    ...
    const [judged] = judgeAgainstBase(live, atBase([verdict("i18n-keys", [], { ok: true })], false));
...
test("once the snapshot has the install, the same check is judged line by line again", () => {
    const live = [verdict("i18n-keys", [DEAD_KEY, OTHER_DEAD_KEY])];
    const [judged] = judgeAgainstBase(live, atBase([verdict("i18n-keys", [DEAD_KEY])], true));
...
test("blindness is about the snapshot's position, not about which check it is", () => {
    assert.equal(blindAtBase(verdict("i18n-keys", []), false), true, "needs node_modules, and none were lent");
    assert.equal(blindAtBase(verdict("i18n-keys", []), true), false, "needs node_modules, and they were lent");
    assert.equal(blindAtBase(verdict("paths", []), false), false, "reads the checkout, so an install was never the question");
    assert.equal(blindAtBase(verdict("paths", [], { measured: false }), true), true, "exited `could not measure`: it never looked");
});

test("with no snapshot at all every finding is the turn's, which is the safe direction to be wrong in", () => {
    const [judged] = judgeAgainstBase([verdict("i18n-keys", [DEAD_KEY])], undefined);
    assert.deepEqual(judged.added, [DEAD_KEY]);
    assert.deepEqual(judged.unsure, []);
});

test("a check that passed on the base and fails now is the caller's even when its lines are unrecognisable", () => {
