...
#   bash _tools/scripts/release/release-prepare.sh 1.135.0
set -euo pipefail
...
# Named from the repo root, which the `cd` above already put us at — counting levels out of this directory is
# the coupling repo-root.sh exists to remove, and the paths check refuses it by name.
node _tools/checks/publish-set.mjs "${PUB[@]}"
...
bash "$DIR/../desktop/build-desktop.sh" "$VERSION" --assemble "$PREBUILT_WINDOWS_DESKTOP_DIR"
