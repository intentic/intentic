...
if [ -z "$release_id" ]; then
  ...
  # beta-lane soak cost.
  release_id="$(gh_create_release "$REPO" "$TAG" "$notes" false)"
fi
...
upload_one() {
  ...
  gh_upload_asset "$REPO" "$release_id" "$f"
}

pids=()
for f in _devices/machine/dist-bin/* _devices/win-launcher/dist-bin/* _sandbox/ic/dist-bin/* _editor/desktop-app/dist-bin/*; do
  upload_one "$f" &
  pids+=("$!")
done
