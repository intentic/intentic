...
// is published via a caller-supplied hook. JWS signatures are ES256 in P1363 form (r||s), which node's default DER
// encoding is not. All network access is injected for in-process testing (acme.test.ts).
...
export const obtainCertificate = async (options: AcmeOptions): Promise<{ certificate: string }> => {
    ...
    const { crv, kty, x, y } = options.accountKey.export({ format: "jwk" });
    if (kty !== "EC" || crv !== "P-256" || x === undefined || y === undefined) {
        throw new Error("the ACME account key must be a P-256 key");
    }
    // Public members only, in RFC 7638's order: a `jwk` header carrying `d` would hand the CA our private key, and the
    // thumbprint hashes exactly these members, lexicographically, with no whitespace.
    const publicJwk = { crv, kty, x, y };
    const thumbprint = base64Url(new Uint8Array(createHash("sha256").update(JSON.stringify(publicJwk)).digest()));
    ...
    const signedPost = async (url: string, payload: unknown): Promise<Response> => {
        ...
        const header = encode(JSON.stringify({ alg: "ES256", nonce, url, ...(kid === undefined ? { jwk: publicJwk } : { kid }) }));
        const body = payload === undefined ? "" : encode(JSON.stringify(payload));
        const signature = sign("sha256", Buffer.from(`${header}.${body}`, "ascii"), { key: options.accountKey, dsaEncoding: "ieee-p1363" });
        const jws = { protected: header, payload: body, signature: signature.toString("base64url") };
