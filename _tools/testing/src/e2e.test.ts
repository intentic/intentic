...
test("what is missing is in the title, which is where the runner prints it", () => {
    stubEnv("INTENTIC_E2E", "1");
    stubEnv("DISCORD_E2E_SENDER_TOKEN", "sender");
...
test("everything named, and the tier runs and hands back its credentials", () => {
    stubEnv("INTENTIC_E2E", "true");
    stubEnv("CLOUDFLARE_API_TOKEN", "cf-token");
    const tier = e2eTier("intentic CLI end-to-end", { enabledBy: "INTENTIC_E2E", secrets: ["CLOUDFLARE_API_TOKEN"] });
