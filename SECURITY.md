...
listener or Visitor chat message, a fetched page, a foreign MCP server's result: that arrives outside the
...
- Findings that require an attacker who already has code execution or a shell in the sandbox. The sandbox runs
  ...
  write, and a workspace with a public Visitor chat or a listener reads it from strangers. Trusting the agent is
