...
- **Groups:** every `_`-prefixed root directory is a package group, and the group is the DOMAIN, not the
  ...
  `_devices/` (runs on the user's own machine), `_search/` (code search), `_deploy/` (the bundled deploy
