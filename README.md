...
- **Tests are type-checked too, by `pnpm typecheck`, not by `pnpm build`.** A package that emits to `dist` excludes `*.test.ts` from its build config and re-includes it in `tsconfig.test.json`; a package that only type-checks uses one config for both. Adding a package with tests and no `typecheck` script fails the guard in `_tools/scripts/check-test-typecheck.mjs`. The testing conventions this protects are in [AGENTS.md](AGENTS.md).
