...
// The restic env-file the backup provider writes (RESTIC_PASSWORD + backend creds), reused rather than
// re-threading secrets onto every service node.
...
// Wrap a stateful service's image bump in a transaction: snapshot the volumes, try the new image, and on a health
// failure roll the image AND the data back, then rethrow. Aborts before touching the running service if the
