...
const valkeySchema = backingSchema.extend({
    // The admin password (requirepass), generated by intentic. The binding provider reuses it to run ACL SETUSER
    // for per-app users.
...
// requirepass is a stable generated secret, so rewriting this file each time is idempotent: no re-key risk, and
// the binding's ACL users + appendonly data survive a recreate.
